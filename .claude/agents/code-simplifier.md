...
**Read your own memory first** — `Glob` `.claude/agent-memory/code-simplifier/*.md` and follow `MEMORY.md`'s index. It records what earlier passes learned about this repo's house style, including shapes that look like redundancy and are load-bearing; skipping it is how the same table gets flattened twice.
