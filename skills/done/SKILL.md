...
- If reviewer found issues → **confirm the diagnosis, apply the fix, then re-run the failing thing and compare the count.** A finding is a claim, not a verdict (grep the call site / re-read the line); don't blind-apply, but don't dismiss either — a real bug a blanket `replace_all` left behind looks identical to a false positive until you check. ⚠️ **A correct diagnosis licenses nothing about the remedy** — a suggested fix can cure the reported symptom and break something the failure was masking, and the new failure wears a different cause's clothes. The loop is confirm → fix → **re-run → count**; a fix whose command you never re-ran is unverified, however obviously right it looked.
