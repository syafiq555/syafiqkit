...
That recovery assumes your commits are reachable from the branch you're standing on, which stops being true the moment the session worked somewhere else — a worktree, a branch it checked out and left, another repo's PR branch. `HEAD` then belongs to work you never touched, so `<base>..HEAD` comes back empty while `git status --short` shows a tree full of files that aren't yours, and the two readings together look exactly like ops-only. Take the commit SHAs as the anchor instead of the branch (`git log --author=<you> --since=<session start> --all --oneline`, or the SHA you already have from committing) and diff those directly; the agents then need reading instructions rather than a file slice, since the files may not exist in the checkout they run in — `git show <sha>:<path>` reads a file on a branch that isn't checked out. "Nothing to review" is never the right conclusion for a session you know wrote code; it means the anchor is wrong, not that the work is absent.
...
A slice plus a focus area is what every agent gets by default, and it reliably buys a clean-looking pass — the agent reads what you handed it, finds it consistent, and says so. Three things turn that into a review worth its cost, and you are the only one who can supply them: the judgement call you are genuinely unsure about (name it, including where your answer conflicts with an existing project rule, so the agent argues the case instead of restating the rule); what you have **already verified**, so a report doesn't hand your own findings back as corroboration; and what you did **not** check and want hunted. The last is the highest-value and the one a slice structurally cannot produce, because its answer lives outside the slice — "this bug class may exist in sibling call sites, I haven't audited them" is what finds the second broken copy that nothing in the diff points at. Where the work already carries a verification story, ask what that story would miss rather than whether it holds: a mutation-tested fix is exactly where an untested branch hides, since no mutation over covered lines can reach it.
...
**While they run, do nothing they were spawned to do.** The verification agenda below is rich and already in your head, which is what makes the interval dangerous: starting it early reads as getting ahead rather than as duplicating the agents. It pays for the same facts twice and destroys the check that makes delegating safe, since a fact you derived inline and then see restated in a report reads as corroboration while confirming nothing. Calls that map onto a scope you just assigned are the shape this takes. The harness re-invokes you on completion, so waiting costs nothing and polling costs the delegation — work on something disjoint, or don't work. Waiting means ending the turn: there is no waiting call, and a `sleep` is blocked by the harness. 📖 `../_shared/references/explore-delegation.md`
...
The checkout is shared by every project, so it may carry another session's work. Settle ownership by comparing each file's modification time against your session's start. Files stamped OUTSIDE your session's window drop out, in either direction: earlier is work that finished before you started, and later — including a timestamp hours ahead of now — is a peer writing as you read, not a file of yours. If the list empties, Gate B did not fire.
