...
⚠️ **A file partition is necessary, not sufficient — agents also share whatever their commands touch.** The partition governs which files each agent edits and says nothing about the test database, dev server port, seeded fixture set or queue they all reach through a command. A suite that drops and re-migrates one shared database destroys a sibling agent's schema mid-run no matter how disjoint their file lists are, and the same collision happens between an agent and you when you run the suite yourself while one is still going. So before emitting, ask what each role's prompt will make it *run*, not only what it will write: where two agents would run the same suite, tell all but one to write code and report, and run the suite once yourself after they land.

The failure is worth naming because it impersonates your own diff. Each contending run dies on a *different* unrelated fixture — a table, a migration, a seeded row your change never touched — so it reads as breakage you introduced, and the cost is concluding your work is broken and "fixing" code that was fine. The tell is that the failing thing **changes between runs** and sits outside the diff. Confirm by checking for an already-running instance (`ps`/`pgrep` for the test command) rather than by reading the error, then recover by resetting the shared resource once, uncontended, and re-running alone. The `git rev-parse HEAD` / `git status -sb` re-read after the fan-out cannot see any of this — nothing is wrong with the tree.
