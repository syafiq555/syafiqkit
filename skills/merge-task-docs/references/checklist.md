...
| Force a flat merged doc past 300 lines by deleting real facts | Condense first; if sources are already dense (not bloated), split into index + `decisions/<theme>.md` instead (Step 4's size check) |
...
| Preserve both docs' Last Session entries | One merged Last Session noting the merge happened — unless a source is contested (Step 4's Last Session step) |
