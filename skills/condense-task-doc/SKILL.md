...
7. **Draft + verify**: execution model is `_shared/references/two-tier-condense.md` — work inline, no spawned agent, using steps 1-6's findings (the row-deletion list, the duplication map, the section plan).
   **Pick the write mode from how much of the file actually changes** — estimate it from steps 5-6's findings BEFORE writing:
   - **Most of the file survives** (targeted row deletions, pointer collapses, a few tightened cells) → **`Edit` part by part.** Re-emitting thousands of unchanged bytes to alter a tenth of them is pure waste, and every untouched line you retype is a line you can silently corrupt. ⚠️ A file that is *already* dense — a doc set that went through a prior condense, or `decisions/*.md` MADR blocks near their ~20-line floor — is nearly always this case.
   - **Most of the file goes, or its structure changes** (narrative sections collapsing into tables, section reordering, a template-conformance rewrite) → **`Write` the whole file.** Partial edits across a restructure leave stale content stranded between hunks.
   ⚠️ **Tell you chose wrong: you `Write`-rewrote a file and its byte delta came out under ~15%.** That is an `Edit` job that cost a full rewrite. Say so when reporting rather than presenting the small delta as the finished result.
   ⚠️ **Verify with a diff against the file's pre-draft state, read every `-` line, confirm the same fact survives in the `+` lines — never verify by re-reading the finished file.** (`git diff HEAD` is the right baseline ONLY if the doc was clean when you started — a doc carrying uncommitted edits needs a pre-draft snapshot, or the other writer's changes masquerade as regressions that were never real.) An invented cause/mechanism reads exactly as plausible as the real one in isolation, and a dropped Next Step leaves a doc that still reads perfectly coherent — only the diff exposes either.
...
- **Write mode is step 7's call, not a blanket rule** — `Write` the whole file only when most of it goes or its structure changes; `Edit` part by part when most of it survives. Never `Write`-rewrite a file to preserve it.
- **A restructure fork goes through `AskUserQuestion`, not inline prose** — condensing needs no permission, but choosing between preserving-vs-deleting, or between competing target structures, changes what the doc BECOMES and is the user's call. Ask it as options at the point it arises (matching `merge-task-docs` Step 2's forks). ⚠️ An answer that names a constraint on HOW ("not a generic name", "keep it under X") governs the whole run, not the one option it was attached to.
