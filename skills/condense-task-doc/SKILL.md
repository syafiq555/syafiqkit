...
- Duplicated facts — if a gotcha lives in Critical Gotchas AND Quick Start AND LLM-CONTEXT, keep it in one place and point to it from the others. Same rule across an index + `decisions/<theme>.md` files: an OPEN item restated with full mechanism in the index, a theme file, AND Next Steps collapses to one canonical statement (Next Steps) + bare pointers elsewhere.
...
7. **Draft + verify**: execution model is `_shared/references/two-tier-condense.md` — spawn a background Haiku agent to do the full rewrite (`Write`, not incremental `Edit`; partial edits on a bloated doc leave stale content between hunks), giving it steps 1-6's findings (the row-deletion list, the duplication map, the section plan) so it isn't re-deriving them blind. ⚠️ **Verify with a diff against the file's pre-draft state, read every `-` line, confirm the same fact survives in the `+` lines — never verify by re-reading the finished file or by spawning a second agent to compare full-text.** (`git diff HEAD` is the right baseline ONLY if the doc was clean when you started — a doc carrying uncommitted edits needs a pre-draft snapshot, or the other writer's changes masquerade as drafter fabrications. Baseline rules: `_shared/references/two-tier-condense.md`.) A drafter's invented cause/mechanism reads exactly as plausible as the real one in isolation; only a diff against the actual source exposes it. A re-read pass on this skill's own dry run caught 1 of 6 real fabrications — the diff-based check is what the shared reference actually prescribes and is cheaper than a second full-file comparison agent.
