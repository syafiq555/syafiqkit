...
2. **Check whether this is actually bloat first** — line count lies, since a MADR restructure grows lines while shrinking bytes. Run `git show HEAD:<path> | wc -c` vs `wc -c <path>`:
   ...
   - Bytes grew from genuinely new ADRs, still >300 lines → not condensable either (every ADR earns its place; step 6 has nothing to cut). **Split** by default, no permission needed — index + `decisions/<theme>.md` per `task-summary/references/templates.md`.
