...
2. **Check whether this is actually bloat first** — line count lies, since a MADR restructure grows lines while shrinking bytes. Run `git show HEAD:<path> | wc -c` vs `wc -c <path>` — **valid only if the doc was CLEAN at session start**; on an already-dirty doc HEAD is a prior writer's baseline, so the delta credits their work to your pass. Dirty → measure `wc -c` before your first write, or use `git show :<path>` (staged). Per `_shared/references/two-tier-condense.md`.
   ...
   - Bytes grew from genuinely new ADRs, still >300 lines → not condensable either (every ADR earns its place; step 6 has nothing to cut). **Split** by default, no permission needed — index + `decisions/<theme>.md` per `task-summary/references/templates.md`. The index keeps **three** things (templates.md L136): Quick Start, **doc-wide operational tables** (Task Status / Bugs Fixed / Critical Gotchas / Next Steps, scoped to cross-cutting), and the routing table. Only theme DETAIL moves down. After splitting, `grep '^## '` the index and confirm those sections survived — no other check here detects a missing section.
...
7. **Draft + verify**: execution model, write-mode choice, and diff-baseline rules are all `_shared/references/two-tier-condense.md` — work inline, no spawned agent, using steps 1-6's findings (the row-deletion list, the duplication map, the section plan). Estimate the write mode from steps 5-6 BEFORE writing. **A doc set that already went through a condense, or `decisions/*.md` MADR blocks near their ~20-line floor, is nearly always an `Edit` job.** **Tell you chose wrong: you `Write`-rewrote and the byte delta came out under ~15%** — say so when reporting rather than presenting the small delta as the finished result.
