...
7. **Draft + verify**: execution model, write-mode choice, and diff-baseline rules are all `_shared/references/two-tier-condense.md` — work inline, no spawned agent, using steps 1-6's findings (the row-deletion list, the duplication map, the section plan). Estimate the write mode from steps 5-6 BEFORE writing. ⚠️ A doc set that already went through a condense, or `decisions/*.md` MADR blocks near their ~20-line floor, is nearly always an `Edit` job. **Tell you chose wrong: you `Write`-rewrote and the byte delta came out under ~15%** — say so when reporting rather than presenting the small delta as the finished result.
