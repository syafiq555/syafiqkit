...
- **Never delete a Next Step** — only remove items marked ✅ or described in past tense in a Bugs Fixed row. This is a bar on SIZE-driven deletion and is right: an open actionable is not bloat.

  ⚠️ **It is not a reason to preserve an item that is OBSOLETE rather than done, and the rule reads as one — which is how a dead blocker survives every pass.** "Done" and "no longer applicable" are different states, and only the first has a marker, so an item superseded by a decision nobody wrote down keeps its `[ ]` and its 🔴 forever while each pass correctly declines to touch it. Measured 2026-09-11: "Request 2C2P production credentials" sat as blocking through three passes of a doc whose platform had moved to a different gateway entirely. **Retire it in place rather than deleting it** — mark it `[x]` with the evidence that killed it (the resolved value, the superseding decision), because a reader who remembers the blocker needs to find out it is gone, and a silent deletion reads as the item having been lost. This is a judgement you are allowed to make, but not one to make from the doc alone: resolve the value first, the way `read-summary` requires before honouring a blocker.
