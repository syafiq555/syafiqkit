...
⚠️ **Reordering a numbered list is a renumber, and a renumber owes its own citations a repoint — deleting one as "dead" is the move that looks like tidying.** Every `rule N` / `Step N` citation in the file now names a different item, and the file still reads as a clean sequence, so nothing in the prose flags it. The condense instinct makes it worse: a cross-reference whose number went stale presents as obsolete cruft, when the reorder you just performed is precisely what made it fixable. Measured 2026-09-15 reordering `hooks/RULESET.md` so its most-failed rule led — one citation was deleted as dead (it named a live pairing between two rules), and a second, in a section untouched by the diff, silently came to point at the wrong rule. **Sweep for the citation syntax (`rule [0-9]+`, `Step [0-9]`), never for the retired number**: the old label is usually gone, so grepping it returns zero and reads as clearance, while the live breakage is a citation carrying a number that still exists and now means something else. Read every hit against what the number names *after* the move. **Tell: you are deleting a cross-reference whose target number stopped matching, in a pass that changed the numbers.**

⚠️ **Cutting a claim's evidence while keeping the claim leaves an assertion the file can no longer support, and it shrinks the byte count exactly like a good cut.** Measured figures and dated incidents are the densest-looking prose in any rule, so they surface first as padding — but a sentence like "the failure is measurable" or "this is the most-failed rule" is *only* load-bearing because of the numbers underneath it. Removing them converts a grounded rule into an appeal to authority, and the next pass, finding an unsupported claim, cuts the claim too. Either keep the evidence or cut the claim it supports; never keep the claim alone. **Tell: your diff removes a percentage, a count or a date from a paragraph whose topic sentence still asserts that something was measured.**
