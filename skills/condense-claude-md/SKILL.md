...
3. **Draft + verify**: execution model is `_shared/references/two-tier-condense.md` — spawn a background Haiku agent to rewrite the file from scratch (`Write`, not incremental `Edit`), giving it step 2's keep/compress/cut scoring so it isn't re-deriving it blind. ⚠️ **Verify with `git diff HEAD -- <file>`, not by re-reading the finished file or spawning a second comparison agent** — a drafter's invented cause/mechanism reads as plausible as the real one until diffed against the actual source.
