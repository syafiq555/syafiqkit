...
6. **Split to a subdir CLAUDE.md when a whole section is subdir-local.** This is a real condensing lever, not just compression: a subdir `CLAUDE.md` auto-loads *additively* on top of its parents (editing `resources/js/routes/X` loads root + `resources/js/` + `routes/`), so a section can move down a level and leave only a one-line `> 📖` pointer behind. But apply the **seam-test** first: move a section ONLY if its rules are both needed in that subdir AND useless elsewhere. A token/money/table rule consumed across many sibling dirs is cross-cutting — splitting it forces you to either duplicate it into each subdir or orphan it, which is worse than a slightly longer layer file. Vertical-slice trees (`app/Domain/*`) usually pass the seam-test; horizontal-layer trees (`components/`, `pages/`, `hooks/`) usually fail it because their gotchas are about shared primitives used everywhere. When you do split, keep the parent's `> 📖` pointer carrying the single highest-cost fact inline — a fresh session won't follow a bare pointer.
   ...
   ⚠️ **Check the seam-test against EVERY real sibling subdirectory, not just the one the section's NAME suggests.** A "Multi-Agency" section reads as `Domain/*`-owned by name, but if its actual content is authorization/scoping code, real usage can concentrate in `Http/Controllers`+`Requests`+`Middleware` instead — a candidate easy to skip because it doesn't sound domain-related. Grep the section's 3-5 core symbols against every plausible candidate directory (`rg -l "<symbol>" <dir> --type=<lang> | wc -l`), not just the intuitive one, and let usage counts decide — a section can fail the seam-test against the obvious guess and pass it cleanly against a directory nobody thought to check.

   ⚠️ **When the seam-test fails, do NOT reach for `.claude/rules/*.md` with `paths:`/`globs:` frontmatter as a substitute.** It looks like path-scoped loading but isn't — empirically (negative-control canary test) those files always load in full regardless of frontmatter, so a section "split" there costs the same tokens as leaving it inline, just with extra indirection. The ONLY mechanism that genuinely scopes load-by-path is a real subdirectory `CLAUDE.md`.

   If no real subdirectory passes the seam-test, check whether the section is FEATURE-specific (not a cross-cutting layer convention) before accepting "stays in the layer file" as final — a feature-scoped debugging/gotcha section can route to that feature's task doc instead (`update-claude-docs/references/structure.md` §6 "second structural lever": task-doc index + `📖` pointer, live-confirmed working via `Explore`/`Plan`'s now-unconditional `/read-summary` discovery, see `agent-setup` D18). Only a genuinely cross-cutting section — no subdirectory owns it AND no single feature owns it — truly has nowhere else to go and stays in the layer file.
...
- **Report line count AND `wc -c` byte count before/after** — line count alone is not a valid completion signal; a table shape where rows never merge (one row per gotcha) can hold line count flat while cutting cell bloat, and reporting only lines makes that real reduction invisible. Always run both, even when the file doesn't look like a `Symptom | Cause | Fix` table on first glance.
