...
3. **Draft + verify** (only if step 2's gate didn't route around this): execution model, write-mode choice, and diff-baseline rules are all `_shared/references/two-tier-condense.md` — work inline, no spawned agent, using step 2's keep/compress/cut scoring. ⚠️ **Tell you chose the write mode wrong: you `Write`-rewrote and the byte delta came out under ~15%.**
