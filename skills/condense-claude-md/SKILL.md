...
5. **Section order** follows the taxonomy in `../update-claude-docs/references/structure.md` §3, which orders sections by how load-bearing each is at the top of a session. That file is the single home for the order; restating it here is how the two copies drifted apart.

"Split" names a shape, not a destination. Two questions decide it, and conflating them is the recurring error: *should this content still auto-load every session* is asked on every pass, while *does it move to a new file* needs an over-budget source. Measure first (`../_shared/references/declared-budget.md`). Under budget, the answer to a section that reads as a wall is the in-place split — `### ` subsections with `{#anchor}`s, every row inline, no new file, grouped by the axis a reader searches on (where they are when the symptom hits — runtime stage, test run, environment) rather than which subsystem authored the row. Over budget, lever #6 or #7 below relocates it. A session once created companion files for a 216-line file well under budget, reading "split by category" as a request about size when it was about organization — if you're about to create a file and the source is under its declared budget, that's the signal you skipped this check.
...
3. **Draft + verify** (only if step 2's gate didn't route around this): execution model, write-mode choice, and diff-baseline rules are all `../_shared/references/two-tier-condense.md` — which also covers when to hand the draft to a `haiku` agent and what the parent session owes afterward — using step 2's keep/compress/cut scoring. A `Write`-rewrite whose byte delta comes out under ~15% means `Edit` was the better fit — most of the file survived unchanged, and a full rewrite of mostly-unchanged bytes is just risk with no return.
...
- A table's shape is a means, not the thing being preserved — collapsing `Symptom | Cause | Fix` to two columns, or to prose, is a legitimate rewrite when the result reads better. What has to survive the collapse is the content of the cell you drop: a Cause cell is where the greppable specifics tend to sit — the exception class a stack trace shows, the exact expression that fixes it, the `See <Class>` pointer — and a row merged without them reads complete, so the loss surfaces nowhere. Renaming columns you're *keeping* is the one move with no upside: it breaks the positional `awk -F'|'` checks below and changes nothing a reader sees.
