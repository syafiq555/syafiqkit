...
2. Mentally score each section: **keep as-is / compress / cut**
   ...
   ⚠️ **Atomic-file gate — run BEFORE step 3, not after.** Skim 3-5 random rows per gotchas table: if none share a topic or restate another row's mechanism, the file has no compress pass to run — skip step 3 entirely and go straight to the seam-test (Restructuring #6/#7). Running the write step anyway and discovering "already atomic" afterward, from the *result*, is the failure this gate exists to prevent — a light pass on an atomic table caps at 1-10% no matter how many times it runs, and reporting that capped number as progress reads as done when the real lever (the split) was never applied. Details on the atomic-file signal: Process step 5's note below.
...
4. Count lines: target ≤200 for project root CLAUDE.md. If still >250 after compressing, check for a section that passes the seam-test (Restructuring #6) and offer to split it to a subdir `CLAUDE.md` before asking which sections to cut — splitting relocates content without losing it, cutting loses it.
   ...
   ⚠️ **"Under the ceiling" and "context-efficient" are different goals — a file can pass Process #4's byte/line check and still be the wrong thing to keep fully inline.** The ceilings above bound *file size*; they say nothing about whether every row in a passing file needs to auto-load on every session in this project. Run the seam-test/split analysis even when both ceilings are already met whenever the ask is about context budget rather than a size violation — including a plain "condense" request pushed back with something like "surely we don't need most of these every session," not only an explicit "make this leaner to read." A bare `/condense-claude-md` with no further phrasing still means frequency-of-use is in scope, not just byte count — don't wait for the user to name the goal before applying it.
5. **Always check `wc -c` alongside line count before reporting done — line count alone is not a valid completion signal.** A `Symptom | Cause | Fix` table has one row per gotcha, so rows never merge: it can sit at the ≤200-line target while cells still run 800+ characters (rule of thumb: ceiling is 40KB for a root CLAUDE.md; a non-root layer file — e.g. `app/CLAUDE.md` — has more slack but the same logic applies). If bytes are still above the ceiling after compressing, apply the Cause/Fix-redundancy rule (Restructuring #4) or run the seam-test (Restructuring #6/#7 per the rule above) in the same turn.
   ⚠️ **If step 2's atomic-file gate already routed around the write, this step still applies to the seam-test/split it triggered instead.** "Already atomic" is never a reason to stop and report "leave as-is" — a large heterogeneous table with no shared mechanism to compress is precisely the shape Restructuring #7's companion split targets. Propose that split in the same turn, not as a follow-up once told the light pass wasn't enough.
   ...
   ⚠️ **Don't trust an "already atomic" verdict from checking only ONE narrow defect (e.g. exact duplicates).** That check can come back clean while the real bloat — a full incident narrative kept alongside the rule, two distinct rules merged into one cell — sits in the same file unexamined. Before asserting "no further compression possible," sort rows/cells by length (`awk -F'|' '{print length, NR}' file | sort -rn | head`) and read the 3-5 longest against the Restructuring #4 checklist directly — a verdict based on one defect scan is not a verdict.
