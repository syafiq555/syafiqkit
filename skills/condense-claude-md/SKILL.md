...
3. **Draft + verify**: execution model is `_shared/references/two-tier-condense.md` — spawn a background Haiku agent to rewrite the file from scratch (`Write`, not incremental `Edit`), giving it step 2's keep/compress/cut scoring so it isn't re-deriving it blind. Then verify per the shared reference before accepting.
