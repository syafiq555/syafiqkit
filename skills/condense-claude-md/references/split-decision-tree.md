...
**Default shape: an index file, not a pointer block.** A split producing more than one companion gets `CLAUDE-<layer>-gotchas.md` as a thin router — subject rows and their destination files, nothing else — with the parent pointing at the index rather than enumerating siblings. Two things make this the default rather than a preference. The parent's pointer block is hand-maintained prose that drifts the moment a later session adds a companion without touching it, and the drift is invisible: the new file is written, correct, and unreachable, while the parent still reads as a complete list. An index file makes adding a companion a one-line edit in the obvious place, and makes the orphan check meaningful — every companion should have exactly one inbound citation, from the index. It also gives the set a subject-named front door, so the layer name that can't organize CONTENT (per SKILL step 4) does the one job it's suited to: routing.

Keep the index a router. Rows say when to read each file in the reader's own symptom vocabulary — the words they'd be holding when they arrive, not the filename restated. A reader who can't tell from the row whether it's their file will open two or three, which costs more than the single dense file the split replaced. Anything genuinely cross-cutting (a constraint every companion is subject to) is the one exception worth inlining in the index, since it belongs to no single file.

**Pointer format:** `📖 See .claude-companions/shared/CLAUDE-<topic>.md`. `shared/` is tracked in git — a companion supporting a checked-in file belongs there, and its discovery depends on being indexed by parent pointers. `local/` is gitignored and is the right home for a companion supporting a local-only file (`CLAUDE.local.md`, `.env`-adjacent notes); never commit those regardless of what the parent's tracking state is.
