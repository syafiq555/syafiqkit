...
**Location**: ask *which repo owns the content* before asking where in a repo it goes — "nearest git-repo root" silently assumes one does, and a companion describing something installed per-MACHINE rather than per-project (a user-scope MCP server, a CLI on `PATH`, a shared credential store, anything reached from every checkout) is owned by none of them. Routing that to the repo you happen to be sitting in produces a correct-looking file the sibling repo can't reach, and the sibling's own session then writes its own copy — two files that drift with nothing to reconcile them, which is the failure the split was meant to prevent. **Tell: you can name a second repo whose sessions would need this file.** Such content goes beside the global `CLAUDE.md` instead (`~/.claude/references/` or `~/.claude/.claude-companions/`, matching whichever that file already cites), cited by absolute path from each repo's `CLAUDE.local.md`. Only content genuinely scoped to one project takes the ladder below.

For project-scoped content: one `.claude-companions/` folder at the nearest git-repo root — not a same-directory file, and not the monorepo root in a multi-repo checkout (each repo gets its own). For the global `~/.claude/CLAUDE.md`, check whether `~/.claude` is itself a git repo (`git -C ~/.claude rev-parse --show-toplevel`) rather than assuming. When it is tracked — a dotfiles/settings repo is the common case — the companion goes **nested** at `~/.claude/.claude-companions/`, because that is the only path the repo's remote backs up, and content written to an untracked sibling is lost with the disk. Only when `~/.claude` is genuinely not a repo does the sibling `~/.claude-companions/` apply. Either way, `shared/` must not be gitignored where it lands. Two subfolders split by trackability:
