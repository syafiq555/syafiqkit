...
⚠️ **The template defines one `## Last Session` and no `Previous Session`, so rotating the old block down is a convention each session reinvents — and the natural move collides.** Inserting a dated `## Last Session` above the existing one and renaming that to `## Previous Session` produces a second heading by the same name whenever a prior session already did it, which breaks anchor uniqueness silently: both render, `Edit` still finds unique anchors elsewhere, and nothing in the doc looks wrong. Rotation is also where the doc grows without anyone deciding it should. Before writing a new session block, grep the doc's existing `^## ` headings and pick a name no sibling holds; then ask whether the oldest block still earns its place, since a session whose facts have all been routed to their typed sections is narration the git history already holds.
