...
| **Git-tracked state doesn't belong in the doc at all** | Never write "committed" / "uncommitted" / "pushed" / "not yet pushed" — `git log`/`git status` answers this and a doc copy is wrong the instant someone commits. Confirmed 2026-07-19: the user's objection to a duplicated "committed" wasn't "grep harder for it," it was that the word adds nothing even said once, even when accurate. Deleting these words is a real, low-risk cut every condense pass should make on sight — not a row-existence judgment call. |
| **Deploy/environment status is one fact, and it DOES belong** | "Deployed to staging" / "LIVE in production" / "prod gate un-approved" is NOT git-tracked — it's a fact about which environment ran the code, and matters (staging has real S3 CORS, local media tests behave differently). This is the one that goes ONLY in Quick Start's state line, with everywhere else pointing there ("see Quick Start") instead of restating it. |
...
| **An OPEN item (a live 🔴/bug/gotcha) gets exactly one canonical statement — everywhere else is a bare pointer** | Applies across an index + its `decisions/<theme>.md` files, not just within one doc. Restating the mechanism (root cause, code path, fix options) in the index AND the theme file AND a Task Status cell AND Next Steps is the same "one fact, one home" violation Layer 1 already names — it just crosses files instead of sections. Canonical home = the item's row in `## Next Steps` (or the owning decision's Consequences under whole-doc MADR); every other mention is `→ see current.md Next Steps`, never a re-explanation. |
| **`⚠️` is reserved for irreversible/destructive consequences only** — data loss, a broken legal/audit trail, a silent prod regression, an unrecoverable action. Not for "surprising," "looks like dead code but isn't," or "worth knowing" | When it shows up every 8-10 lines it stops meaning "danger" and starts meaning "sentence" — a reader can no longer triage by glance, the entire reason the marker exists. Before adding one, ask "if this specific consequence happens, is it unrecoverable?" — if no, use bold text instead. |
...
`current.md` should stay **under 300 lines**, measured by byte size (`wc -c`), not line count — a whole-doc MADR rewrite can legitimately grow lines while shrinking bytes (a dense table cell becomes several short ADR Consequence bullets). Compare `wc -c` against the version you started from before forcing a condense; flat/shrunk bytes means the doc restructured, not bloated. ⚠️ **`git show HEAD:<path>` is the right baseline only when the file was CLEAN at session start** — an already-dirty doc makes HEAD's copy someone else's starting point, so the delta you report is a prior writer's work plus yours, and a byte-flat pass reads as a large reduction. Capture `wc -c` BEFORE your first write, or diff `git show :<path>` (staged) — see `_shared/references/two-tier-condense.md` for the full baseline rule. **Tell: you never measured the file before editing it.** Already over budget by bytes → condense FIRST via `condense-task-doc` rather than hand-rolling — its row-existence pass (delete gotcha/decision rows discoverable-from-code) is the step most likely skipped when improvising; sentence-tightening alone on a 40+-row doc isn't a real reduction. ⚠️ **Budget the SET, not the index**: once a doc is split, `decisions/*.md` routinely outweighs `current.md` several times over, so an index measured alone reads healthy while the feature's docs are the real problem. `cat current.md decisions/*.md | wc -c` is the number that matters.
...
Litmus tests before finishing (also see Validate §5.8): (1) grep the doc for `committed`/`uncommitted`/`pushed` (case-insensitive) — ANY hit is a delete, not a dedupe, per Layer 1. ⚠️ One carve-out: an MADR `**Status**: committed` field is a decision-lifecycle value (`committed | planned | debating`, per `references/templates.md`), not git state — no git command answers whether a decision was accepted. Leave those; delete every other hit. (2) grep for its 2-3 most critical phrases, plus its deploy/environment word (`deployed`/`staging`/`prod`, case-insensitive) — >1 section containing the deploy word means collapse the extras to Quick Start's state line per Layer 1; (3) scan for sentences with 2+ parentheticals or commit hashes outside Last Session — rewrite them.

## 1. Resolve Path

⚠️ **Before writing — scan or explicit path alike: do you own these docs?** Settle it by diff CONTENT, never by status plane (`_shared/references/diff-ownership.md`) — auto-staging makes your own writes indistinguishable from another writer's staged work. The question is whether this session's own content traces this diff. A background `Agent` still running, `git status` showing `tasks/` files you never touched (a parallel session), and uncommitted doc edits predating this session (no rival writer, so nothing looks contested, yet an unscoped scan rewrites those docs just the same) are what "not fully yours" has looked like in practice, not a closed list — a case resembling none of them, like two sessions having edited the *same doc* so the content itself is a mix, still needs the same question asked, and once that's true no pathspec separates the two. Any of these → **the doc is contested for the rest of this run** (this is what "§1 guard fired" means in §4's table): do NOT run the multi-domain scan, and on an explicit path take §4's guard-fired branches instead of its overwrite mandates. Verify read-only instead (report gaps for the owner to fix), or scope to the doc you actually own. A writer reads a file when it starts, not when it finishes.
...
1. **Code changes**: `git status --short` (every repo the session touched — see the sibling-repo rule below) → infer domains from file paths. ⚠️ Not `git diff --name-only` — it hides staged AND untracked files, so it returns **empty** once you've staged, and you'd conclude "no code changed" and skip every doc. A nothing result for work you just did = the blind spot, not a clean tree. ⚠️ If it's empty because the session's work is already **committed**, that's the same blind spot with a different cause — count off the session's base commit instead: `git diff --name-only <base>..HEAD`, where `<base>` is HEAD at session start (or the merge-base with trunk).
...
⚠️ **A SIBLING repo has its own `tasks/` tree, and step 1's scan does not reach it.** Two independent checkouts in sibling directories are not the nested-sub-repo shape the commit/ship skills describe — no `git -C <subdir>` walk finds them, and the harness only walks the tree from the working dir. So a feature spanning both repos gets its originating side documented exhaustively while the other side's doc is never opened, and nothing fails: the doc you *did* write is complete and correct. Whenever the session's work touched a second repo **at all**, re-run the scan from that repo's root and update its own `tasks/**/current.md` in the same pass (mirrors `read-summary`'s SIBLING REPO step). **Tell: you edited or committed files in two repos and opened a task doc in only one.**
...
## 2a. When Merging, Renaming, or Reorganizing (user requests `merge A into B`, a folder rename, or restructuring the doc set by a new axis)

Cold-path — read `references/merge-rename.md` for the full process, including the reorganize case (splitting many docs onto a different axis, e.g. platform → feature). Merging delegates to `syafiqkit:merge-task-docs`; renaming is `git mv` + reconcile every back-reference. **NO redirect stubs** either way — delete the source outright, the gate is 0 stale references.
...
3. **`## Next Steps` is grouped by KIND of work** — regroup an ungrouped or date-grouped list on sight, whatever its length. Step 2 is blind to this (no table columns to compare), so it needs its own check. Vocabulary: `references/templates.md` "Next Steps".
...
| `## Quick Start` | ⚠️ **MANDATORY on every update** — rewrite entirely (see below). **§1 guard fired** → additive only (`_shared/references/contested-doc-sections.md`). |
...
| `## Next Steps` | Remove done, add pending. **FLAT `- [ ]` checklist — never sub-headings grouping items by WHEN they were found ("**this session's findings:**", "**earlier:**"); that drifts it into a changelog.** Order by priority if needed; session provenance goes in `## Last Session`, not as structure here. ⚠️ **The ban is on the temporal axis; grouping by KIND of work is REQUIRED — enforce on sight, at any length.** An ungrouped or date-grouped list gets regrouped the moment you touch the doc, not at a size threshold. Canonical group vocabulary + skeleton: `references/templates.md` "Next Steps". A by-kind axis survives the next edit; a date-based one is stale on arrival. Test any heading you invent: still true in three sessions? **A user's scope decision about an item (defer, decline, deprioritize, block-on-X) is a decision to capture — write it into the item with its reason; chat-only acknowledgement is lost next session.** Keep an item's markers consistent with its captured status: a priority/urgency marker on an item the user just deferred contradicts itself — re-mark to match. |
| `## Last Session` | **Overwrite in place — ONE session only, ≤5 bullets, ≤2 lines each.** Delete the previous session's bullets entirely (never append a dated bullet below them — that's a changelog). Before deleting, fold any still-load-bearing fact into its proper Decision/Gotcha row. **§1 guard fired** → do NOT overwrite; route your facts to their typed sections instead (`_shared/references/contested-doc-sections.md`). |
...
⚠️ **MANDATORY** — place immediately after the `# Title` and before `## Overview`. Rewrite on EVERY update (not append) — unless §1's guard fired, then additive only per the table above. A fresh agent reads ONLY this section before starting work. If it can't act from Quick Start alone, the section is insufficient.
...
4. Next Steps has no stale completed items, and is grouped by KIND of work — no flat list, no group named for when its items were found, no empty groups
5. No rows deleted
6. Back-references reconciled (§6) — no roadmap/index/`Related:` doc still mirrors an out-of-date status for the feature you just updated. **Count the repos the session's work touched against the repos whose docs you opened — if the first number is larger, §1's sibling-repo rule did not fire and the pass is incomplete.**
...
1. `Grep tasks/**/*.md` for the updated doc's path **and** its feature name/vocabulary — **in every repo the feature spans, not just the one you edited.** A cross-repo feature's loudest mirrors live in the *other* repo's tree, where a same-repo grep structurally cannot reach them.
...
⚠️ **The feature-name grep above misses two other reconciliation cases — grep for each one's own vocabulary, not the feature's.** (1) If the session FIXED a bug, a doc describing it may never mention the feature at all, only the defect — grep the symptom/flag/command the gotcha names (`RELOAD_NGINX`, the error string) plus its hedges ("still", "until they're fixed", "can never"); every hit asserting the bug is LIVE must flip to fixed, with the *reason* it's now safe. ⚠️ Run the command that settles it before flipping, and flip in the other direction too — a doc claim contradicting your session's experience is as often right as stale, and an unverified "correction" discredits a source that was correct while reading as settled to every later session. A stale status is out of date; a stale gotcha **actively misleads** — the next session re-fixes a solved bug or routes around a non-problem. (2) If the session MOVED a fact — split a doc, extracted a section, renamed an anchor — grep the FILE/ANCHOR name instead, since a move leaves every claim **true** and only the *routing* wrong, so a claim-based grep finds nothing to fix. Nothing 404s; the emptied file still resolves as a router, and every `📖 <file>` that promised a fact now lands the reader where the fact isn't: `grep -rn "escrow-engine.md" tasks/` and repoint each hit at the leaf that now owns the fact, prioritizing `Gotchas:` teasers and `Next Steps` items (their contract is "one-line fact + the file with detail", so a reader may act on the teaser alone), then mark the emptied file a router in the index.
