...
2. Verify each existing section's internal structure matches the template — table columns, field names, order — and fix non-conformant structure in place. What counts as drift is a section going missing, not a shape that differs — a split axis fitting the domain (Hosting/Build-Pipeline) or columns carrying actual gotchas are correct as-is. Reshape only when the current shape loses something. ⚠️ This is deliberately the opposite of what the CLAUDE.md skills do — they enforce house style on any file they touch (`../../_shared/references/adopt-vs-impose.md`), and that rule does not extend here. A task doc's shape follows its domain, so a split axis or a column set that fits the work is correct as it stands.
