...
2. Verify each existing section's internal structure matches the template — table columns, field names, order — and fix non-conformant structure in place.

   What you're fixing and what you're leaving alone both follow from who decides the thing. The template decides which sections a doc has, so a heading that is missing, renamed, or borrowed from another doc is drift and this gap-check is where it gets corrected. A reader's situation decides what a section needs to hold, so a split axis fitting the domain (Hosting/Build-Pipeline) or columns carrying actual gotchas are already right — reshaping those needs a reason of its own, that the current shape loses something, since non-conformance alone isn't evidence about a choice the template never made.

   That is also the extent of the difference from the CLAUDE.md skills, which is narrower than it first reads. They convert a file's tables and rows into the plugin's own patterns (`../../_shared/references/adopt-vs-impose.md`), and a task doc's gotcha tables are exempt because their columns answer to what a reader is holding. Its headings are not exempt, and never were.
