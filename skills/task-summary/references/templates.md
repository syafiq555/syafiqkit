...
⚠️ **Default rule (no user ask required)**: once a whole-doc MADR is still >300 lines after legitimate ADR growth (not restructure artifacts — see `condense-task-doc` Step 2), split into an index + `decisions/<theme>.md` files as part of the normal flow. The user can still request a split below the threshold. **One exception**: if the doc's own Quick Start/Next Steps already deferred this exact split 2+ times as "disproportionate for this session's scope," treat that history as a real scope signal and confirm before proceeding — it usually means multiple files split at once, not the routine single-file case this default assumes.
