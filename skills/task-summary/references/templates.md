...
⚠️ **Measured cost — a MADR block is not free, even as the default.** An MADR block runs roughly 18-20 lines vs the ~1 line an escape-hatch table row takes — that's the entire size budget for several other facts, and the reason the escape hatch exists. Apply the test honestly (would Rejected actually come up empty?) rather than defaulting to MADR out of habit for a decision that never had a real second option.
...
⚠️ **Whole-doc MADR is NOT priced like per-block MADR — judge the rewrite by bytes, not lines, and expect either to move.** The "+18-20 lines each" cost above measures ADDING a block ALONGSIDE existing Decisions + Gotchas tables; a whole-doc rewrite REPLACES those tables — each gotcha moves into the Consequences of the ADR that created it, removing the cross-section duplication of a fact that was both Decision (why) and Gotcha (what-breaks). Measured twice: a 13-decision/27-gotcha doc went **307→284 lines** (shorter, not +200); a denser 13-decision/62-gotcha doc went 275→470 lines but 54.3KB→49.0KB (`wc -c`) — a wide table cell wrapping 400+ characters behind one pipe becomes several short bullets in Consequences, more newlines but fewer total characters. Lines and bytes can diverge either direction depending on source density. **Report both deltas after a rewrite** and judge success by bytes (or "no fact restated, no fact lost"), not by whether lines crossed 300 — that budget is calibrated for standard-structure docs, not decision logs where structural overhead (~6-8 fixed lines per ADR) is paid once per decision regardless of content density. A decision-heavy doc where most gotchas trace to a specific decision is a GOOD candidate; a gotcha-heavy doc whose traps are environment/deploy noise (not consequences of a choice) is a BAD one — those gotchas have no ADR to live under and end up in a catch-all section anyway.
...
⚠️ **A split silently invalidates every pointer INTO the file you split — grep its filename and re-aim them, in the same write.** The file that splits cannot fix the files that point at it, and this reconcile is invisible from inside the split: nothing 404s, no claim goes false, the emptied file still resolves. It just becomes a **router**, so every `📖 <file>` that promised a fact now lands the reader somewhere the fact isn't. Worst hit are LLM-CONTEXT `Gotchas:` teasers and `Next Steps` items, whose entire contract is "here is the one-line fact, here is the file holding the detail" — a reader (or an agent optimizing tokens) may act on the teaser alone rather than re-navigate. So: `grep -rn "<split-file>.md" tasks/` and repoint each hit at the **leaf** that now owns that fact; keep only pointers genuinely aimed at what stayed behind (Overview, Architecture, Files, Task Status). Then label the emptied file a router in its own Sub-Files table and in the parent index, so the next reader doesn't stop there. ⚠️ Splitting a file that is **already a leaf of another index** rots the grandparent too — check every level above, not just the immediate parent.
