...
What controls density is what gets admitted to a file and whether it's hot-path (inline) or cold-path (`references/`), not restyling prose that already reads fine. Leave a rule's wording alone unless it fails the capture filter or its position is wrong. This governs every mode, Capture included.
...
This skill is long enough that a compacted session may keep only roughly its first half — if the six Capture steps (scan, route, write, prune, validate, sync, named once here) aren't in your context by the time you reach Step 3, re-read this skill rather than improvising. Writing nothing is a legitimate outcome — most signals shouldn't become entries, and "no capture: corrections were applications of existing rules" is a real report. Prefer tightening an existing line over adding a neighbor, because the file's health depends on what arrives, not on how well each arrival is worded.
...
⚠️ **The four classes above all ask what went WRONG, so working them to exhaustion feels like a finished scan and the fifth never runs.** It is the odd one out by construction — a credential is something that went *right* — and it sits last, after the point where the list reads as closable. Report the fifth class explicitly even when empty — "no new machine context: nothing was reached this session that a fresh one couldn't" is a real answer and an unstated one is indistinguishable from a skipped step. If the session touched a live system (connected to a host, queried a database, entered a container, read a token or path), the omission matters.
...
House style in `references/structure.md` applies to any CLAUDE.md this skill touches, including in someone else's repo — a restructure asked for one, and a pass that defers to how the file was already written delivers nothing. An existing file's uniformity is not evidence anyone chose it; it is what one pass produces.

Inventory before touching and diff after; say in one line what you restructured. Rules only disappear via the capture filter (derivable, linter-enforced, feature-specific), never because they didn't fit the shape. 📖 `../_shared/references/adopt-vs-impose.md`

## Rewrite mode: Delegation safety

When a Rewrite is delegated, the inventory claim is the only claim its self-assessment cannot be trusted for. Take your own `cp` BEFORE dispatch and diff after — an inventory existing only inside the agent's context is unfalsifiable once the agent returns. A rewrite legitimately rewords, so a rule can survive carrying none of its original tokens.

Before deleting, ask: is my justification that something else in the repo already contains the string? If yes, stop — the codebase holds the value, the rule holds why it matters.
