...
### Before step 1: is this file unstructured, or structured differently?

A mature CLAUDE.md that a team has maintained for a year usually has a convention of its own, and the steps below cannot tell that apart from a file that never had one. Both present as "not our house style," so the normalize instructions in steps 3 and 4 fire identically on each — and on the first kind they destroy a deliberate, working decision while every check still passes, because step 7 only proves the *rules* survived and says nothing about the form they arrived in. The owner then finds their file reshaped by a tool they ran to tidy it.

Read the whole file first and ask whether its shape is **consistent**. A file that solves the same problem the same way each time — every gotcha as a paragraph, or every rule as a numbered list, or its own table columns used uniformly — is telling you a person decided that. Apply `task-summary`'s test, which is the same judgement one artifact over: **what counts as drift is a section going missing, not a shape that differs. Reshape only where the current shape loses something.** Where the file's own convention carries a rule perfectly well, adopt it and write any new content to match; the house style is the default for files with no convention of their own, not a standard to convert working files to.

That leaves three things always worth fixing regardless of convention, because they are losses rather than differences: a rule filed where a reader won't find it, a missing `{#anchor}` that breaks an inbound pointer, and content the capture filter excludes outright. Everything else needs a reason beyond "it isn't how we write them."

Say which reading you took, in one line, before the rewrite lands — "adopting the file's own numbered-rule convention" or "no consistent convention found, normalizing to house style." An unstated choice here reads afterwards as an accident, and the owner cannot tell whether you judged or defaulted.

📖 `../../_shared/references/adopt-vs-impose.md` — the same judgement as it applies to task docs and condense passes, plus the three repairs that land whichever reading you took.
...
4. **Normalize formatting to house style** — free-form bullets → `❌/✅` rows; debugging notes → `Symptom | Cause | Fix` rows; add missing `{#anchor}`s; strip line numbers down to file+symbol; delete session storytelling. **Only where the gate above found no convention of the file's own.** Where it did, the conversions in this step are the ones to skip: the anchors, the line-number stripping and the storytelling deletion still apply (all three are losses), while the bullet and table reshaping is what you agreed to leave alone.
