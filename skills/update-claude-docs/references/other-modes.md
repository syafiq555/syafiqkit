...
1. **Read `structure.md` in full first** — it holds the hierarchy, capture filter, section taxonomy, formatting conventions, and the template family. Pick the template that matches the target (root-router / domain-or-layer / subdir / global).
...
4. **Write in house style** — LLM-CONTEXT header, `{#anchor}` on every `##`, `❌ NEVER / ✅ INSTEAD` and `Symptom | Cause | Fix` tables, file+symbol references (never line numbers), sections in taxonomy order. Cross-reference the parent layer for shared concepts (`> Schema: parent CLAUDE.md #{plans}`). ⚠️ Strip tool-output wrapper artifacts before writing — a `Read` result wraps file content in `<content>` tags, which can ride into the `Write` payload as a literal trailing line.
...
6. **Validate** (§5 checks apply): anchors present + unique, tables well-formed, no invented rules, no secrets (those go to `CLAUDE.local.md` by name only), cross-refs resolve, and the last line is real content rather than a `</content>` tag that rode in from a `Read` (`tail -c 40 <file>` — step 4's warning).
...
1. **Read the target file AND `structure.md` in full.**
...
3. **Re-section to the taxonomy order in §3** — read it there rather than from a copy here, since that order has already drifted once between two files restating it. Move each existing rule into its correct section. If the file has no Architecture section at all (common in a gotchas-only file that grew incident-by-incident), that's a gap to fill, not just a reorder — derive it from the real contracts/sibling classes (e.g. a "4 mutually-exclusive-precondition sibling Actions" table), never invent structure that isn't in the code.
4. **Normalize formatting to house style** (§4): free-form bullets restating a constraint → `❌/✅` rows; debugging notes → `Symptom | Cause | Fix` rows; add missing `{#anchor}`s; strip line numbers down to file+symbol; delete session storytelling. ⚠️ **Tabular is not the default direction** — apply SKILL.md's prose-vs-table test (`SKILL.md`'s "New signals → Add entry") to EVERY existing table row too, not just new prose: a row whose Fix/✅ cell carries reasoning ("because X, so Y") converts to prose stating that reasoning, and only a bare lookup value stays tabular. Re-check this after finishing one section before moving to the next — fixing it in Critical Rules doesn't carry over to Gotchas automatically.
...
8. **Validate**: diff your rule-inventory (step 2) against the rewritten file — every load-bearing rule still present (possibly relocated), zero dropped. Then §5 checks. Also run the leaked-tag check for step 4's warning — confirm the file's last line is real content and not a `</content>` tag that rode in from a `Read` (`tail -c 40 <file>`, or `git diff HEAD -- <file>` and read the final `+` line) — AND re-run step 4's prose-vs-table check (`SKILL.md`'s "New signals → Add entry") against every section you touched, one final pass across the whole file rather than per-section as you went — the test is easy to apply correctly in the section you're focused on and forget the moment you move to the next.
