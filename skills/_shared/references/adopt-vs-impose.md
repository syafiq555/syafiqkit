...
This file is about CLAUDE.md. Task docs (`tasks/**/current.md` and `decisions/*.md`) answer to their own template in `task-summary`, which governs their headings as firmly as this file governs a CLAUDE.md's — what it doesn't govern is the inside of a section, where a split axis or a column set is chosen for the reader's situation rather than for conformance. That is why the table-and-row conversions above stop at a task doc's gotchas and go no further: those columns are already answering a question this file doesn't ask. `task-summary/references/creating-and-updating.md` §"Gap-Check and Structure-Check" owns the full distinction.
