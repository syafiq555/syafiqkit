...
7. ⚠️ **A rule surviving is not the same as its load-bearing appendages surviving.** A `📖` cross-reference, a "this already happened" precedent example, or an exact command/value is easy to cut as connective tissue because it's usually a short trailing clause, not a rule of its own — the sentence around it still reads complete without it. Check each surviving rule for whether it USED to carry one of these and still does, not just whether the rule's core claim is present.
8. Any dropped rule, changed number, stale-but-surviving fact, dropped pointer/example/exact-value, or scope creep → fix directly.
9. Report `wc -lc` deltas — ratio should drop or hold flat, never rise.
