...
When delegating, extract every backtick-quoted identifier and bare number from the pre-draft original **before dispatch**, and put that list in the agent's prompt as the content it must not lose. That is what the extraction is for. Do not re-run it against the rewrite afterwards as a verification: a token diff over a rewritten file returns a count set by tokenisation rather than by damage, and it has produced a wrong verdict on every recorded occasion. Verification is a sectioned read of the original against the rewrite — 📖 `../../haiku/references/verifying.md`.

Confirm the bytes moved before judging anything (📖 `verifying-a-write-landed.md`), since a pass that wrote nothing loses no identifiers either. And a rewrite can keep every token while restating a fact into falsehood — relabelling kebab-case keys "camelCase", calling a hyphen-vs-underscore mismatch "case-sensitive" — so settle any factual claim the rewrite introduces in new words against the actual code, which is a question about the codebase rather than about the diff.
