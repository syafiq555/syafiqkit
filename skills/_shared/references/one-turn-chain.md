...
Everything above describes a stop the model elects. A chain can also be severed involuntarily, and the guards written for the voluntary case do not reach it, because they all assume the turn is still running. Two things sever it: a context compaction between steps, and **new work arriving from the user mid-chain** — a bug report, a screenshot, a "this is still broken". The second is the more frequent and the better disguised, because answering it is unambiguously right: the work is real, it is urgent, and attending to it feels like the opposite of abandoning something. What is abandoned is the chain, silently, since the steps entered context before the interruption and are recalled afterwards rather than re-read.

⚠️ **The tell is that the interrupting work is itself completable.** Fixing it produces its own satisfying endpoint — tests green, a screenshot confirming it — and that endpoint gets written up as if it were the turn's conclusion, because at that moment it is the only outstanding thing in view. The parent workflow's remaining steps are not contradicted by anything; they are simply not present. So: finish the interrupting work, then ask what was running when it arrived, resume at the step you left, and re-read the skill body rather than working from memory of it — the fix displaced it, and several steps' worth of file edits sit between you and the last time you read it. A wrap-up interrupted three times is still owed its Output block.
