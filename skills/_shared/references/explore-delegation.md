...
**Every multi-file search needs one non-negotiable prompt clause: return raw output, do not rank/filter/summarize.** A filtered return smuggles judgment out of the main loop, defeating the split.
...
The no-prose rule reads as though silence were the natural default, and for many harnesses it isn't: a background-job or status-tracked system prompt commonly asks for one line of approach *before* acting, and an output-shape convention may ask you to lead with the action. Both are correct everywhere else and meet this rule head-on at exactly one point, so a session following its harness walks into the violation while doing as it was told — which is why the failure recurs in sessions that are otherwise being careful. Pay that convention on the message where you settle the partition, then let the next message carry only calls. The sentence explaining the batch and the batch itself cannot share a message.
...
No `Explore`-capable context (inside an agent that can't spawn agents)? Run the same search directly in Bash instead.
