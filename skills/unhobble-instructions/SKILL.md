...
📖 `${CLAUDE_SKILL_DIR}/references/target-types.md` — what counts as a genuine fact per target class, and where the boundary runs through a file rather than around it. Read it when the target isn't an obvious SKILL.md; the fact-vs-constraint test is constant, but what it protects differs by class.
...
Content fails routing when it belongs in a reference file, behind a pointer, or deleted outright — versus staying resident. The question is whether it governs a routine choice (resident, read preemptively) or only arrives once someone holds a failure (pointer, consulted on demand). A catalog — error strings, commands, config paths — usually moves to a reference file. A rule stating derivable information (`ls` would show it, `--help` documents it) gets cut outright.

Safety-critical prohibitions ("never edit generated files", "never push to main") stay resident always; their cost is fixed and their value irreplaceable.

📖 `${CLAUDE_SKILL_DIR}/references/routing-content.md` — the four-question routing test, how a file's delivery shape decides what moves are possible, and the symptom-index clustering method for multi-tier files. Read it when routing means moving content to a companion rather than deleting it outright.
...
⚠️ **Before cutting a rule as derivable, check where else it actually lives — "a reader could look this up" and "a reader can look this up" are different claims.** Derivability in principle and its remaining copies in practice are two questions; a rule stating an obligation the codebase cannot show you (a mirror in another repo that no CI reaches, a manual step deliberately kept out of a deploy path) usually has exactly one home. A grep across the repo's other docs settles it in seconds. Where the target is the sole home, the rule stays resident whatever its shape, because the cheapest correct version of it is still the only version.
