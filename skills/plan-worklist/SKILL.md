...
1. **Load the product's task-doc context via `read-summary`**, scoped to whatever domain/project the list belongs to. `product-reviewer` needs the doc's stated intent to tell a deliberate scope-cut from a real gap, so skipping this makes its judgment worthless.

   Reading the docs yourself instead — a `head`, a `cat`, a couple of greps — is the tempting shortcut, and it drops the three things the skill actually does: discovering *which* docs are relevant by content, following their `📖` and `Related:` pointers, and reading each one whole. A truncated read lands in the front matter and the file tables, where a doc names its architecture in a header phrase but explains it further down. You come away holding the right vocabulary and the wrong model — which is indistinguishable from having read it, until a later step routes on that model and reaches for the wrong tool or the wrong layer entirely. Treat "already loaded" as true only when a full pass ran for *this* domain earlier in the session; a partial read from a few turns ago doesn't count.
