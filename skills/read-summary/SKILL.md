...
- **Investigation** — a question about current state → read the relevant docs first, then investigate. Multi-image requests carry multiple claims; enumerate what each image is evidence *of* before answering. Before sending a conclusion, verify you answered the question the user asked, not an adjacent one. A finished investigation that found defects: report the finding and blast radius, then stop. Deciding which findings matter, when to fix, and where to record them is the user's call — present it as `AskUserQuestion`, not a yes/no.
