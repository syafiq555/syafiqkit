...
⚠️ **Reading the task doc is the MANDATORY FIRST ACTION whenever this skill applies. If you have not read the matching `current.md` (+ its `decisions/*.md` files, if it's a split index) + every CLAUDE.md on the path to the files in play (layer, subdir, domain — see step 5), you have not run this skill — you've ignored it.** There is no path through this skill that answers, investigates, or implements without first completing the discovery and reading the doc. This holds even when you think you already know the answer, even for a one-line "quick check", even when the question seems too small to need context. The whole point of the skill is that the docs carry decisions, gotchas, and vocabulary your prior knowledge does not — skipping the read defeats its only purpose. The first tool call must be the Glob/Grep discovery (or a direct Read if given a path), never a query, edit, or answer.
