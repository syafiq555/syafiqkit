...
- An **existing skill that's close** should be extended rather than forked. Grep the skills dir for the concept's vocabulary before writing — two skills covering adjacent ground both trigger weakly, and the user gets whichever matched more keywords. ⚠️ **A clean grep is weak evidence here, because the sibling that overlaps you names the CATEGORY rather than your feature.** You search your own vocabulary; it claims the general word, so nothing matches and the check reads as passed. Measured 2026-09-17: a new `continue-session` skill grepped clean while `tackle`'s description already claimed "continuation" and "let's continue" — caught only by reading sibling descriptions directly, after the skill was written. So read the `description:` of every skill in adjacent territory before drafting, not just grep for your own words; `references/trigger-testing.md` §"Check for hidden overlap" has the rest, but the check itself belongs here, before there is a file to defend.
