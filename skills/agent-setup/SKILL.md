...
- `Explore` is read-only — no `Write`/`Edit`; it reports locations, not opinions. `Plan` recommends without implementing but is granted `Write` narrowly, restricted by its own body text to `~/.claude/plans/<slug>.md` only — see the naming-exception callout above for why both need `disallowedTools` alongside `tools:`.
