...
⚠️ **A Bootstrap row citing a *section* of a doc goes stale when that doc is restructured, and unlike a moved file it leaves the path resolving perfectly.** `docs/ARCHITECTURE.md` still exists, the row still reads as live, and the section it promises — an open-questions table, a named subsection — is simply gone or renamed, so the agent is sent to fetch something that will not be there and nothing anywhere errors. This is the common case rather than an edge one, because a doc gets reshaped far more often than it gets moved, and the reshaping session has no reason to think about the fleet. Measured 2026-09-01: two documents were rewritten to standard templates hours after the agents were wired to them, and four rows across `Plan`, `product-reviewer`, `code-reviewer` and `claude-md-pruner` pointed at sections that no longer existed. So verify a section citation against the doc's actual headings rather than against the file existing — and where a row characterises what a doc *contains* (a tagging convention, a fixed template, a deliberately-kept record), re-read that too, since a doc can keep every heading and change what a reader is allowed to conclude from it.
...
⚠️ **A newly created agent is not dispatchable in the session that created it, and the error blames the file rather than the timing.** The harness reads `.claude/agents/` when the session starts, so `Agent(subagent_type: "code-reviewer")` right after writing that file fails with *"Agent type not found. Available agents: …"* and the generated agent absent from the list — which reads as malformed frontmatter, and sends you auditing a file that is completely correct. `/reload-plugins` does not fix it either; it reloads plugins, and a project agent is not one, so its success message is genuinely unrelated. Confirm the frontmatter parses if you like, then stop: the agent registers on the next session, and there is nothing to repair. This matters most where the two steps naturally adjoin — running `/done` immediately after a setup pass is exactly the sequence that produces it, and the fallback is to dispatch `general-purpose` with the agent file handed over as its brief.
