...
- [ ] ⚠️ **Field-level drift**: a single agent summarizing "diff all 8 files" in prose reliably drops this — run the field-level greps yourself. A short summary budget makes an agent report structural sections as the unit of comparison and skip differences inside a single line (a `description:` that's merely thinner, a per-tool inline comment copy-pasted from a different agent's template line). Both are real drift a prose diff summary will call "no drift" because nothing whole is missing. For every generated file: `grep "^description:"` both copies and read them side-by-side in full (not just "present/absent"); `grep -n "  - Agent\|# lets this"` both copies and confirm the comment names THIS agent, not a copy-pasted one. Do this directly, not via a delegated summary
