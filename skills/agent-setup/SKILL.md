...
- **A detailed, code-specific prompt is NOT a signal to skip task-doc discovery.** `Explore` and `Plan` in particular receive open-ended free-text prompts (unlike `code-reviewer`/`code-simplifier`/`product-reviewer`, which trigger off a `git diff`). Observed failure: a prompt that already names exact files/methods/questions about a flow (e.g. "trace OAuth authorize-page replay behavior — read `pages/oauth/authorize.tsx`, show `sessionInit()`...") reads as a fully-scoped code trace, so the agent skips straight to CLAUDE.md reads and never calls `/read-summary` — even though that level of prompt detail usually means a task doc exists (the caller got the detail from somewhere) and the flow had prior documented decisions. `Explore.md`/`Plan.md`'s Bootstrap section must include an explicit line countering this: a request naming a flow/feature runs `/read-summary` FIRST, before any CLAUDE.md read, regardless of how code-specific the prompt already looks.
...
- [ ] `Explore`/`Plan` Bootstrap explicitly states that a detailed, code-specific prompt is NOT a signal to skip `/read-summary` — free-text prompts naming exact files/methods for a flow still run task-doc discovery first
