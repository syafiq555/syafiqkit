...
- `mcp__ide__getDiagnostics` in tools — for `code-reviewer` + `code-simplifier`. **Omit for `product-reviewer`** — it judges product completeness, not type correctness. `task-builder` has it implicitly (no `tools:` line = full set)
- **`task-builder` omits `tools:` entirely** — the one agent with no allowlist, so it gets `Agent` plus everything else. Per CLAUDE.md the enum is fixed and un-appendable: adding a `tools:` list back to it would silently revoke `Agent`. Its file-partition discipline lives in the body's Scope Rules, which are therefore load-bearing, not advisory
...
- `browser-verifier` is **read-only** — NO `Write`/`Edit` (`disallowedTools: [Write, Edit]`), else it "fixes" its way to a green run instead of reporting the bug. Three load-bearing rules must survive any template edit: (a) **assert the effect, never trust the tool's success message** — `resize_window`/`save_to_disk` both report success while doing nothing, so back every claim with a measured `innerWidth`/`ls`'d path; (b) **never attribute a claim to the user they didn't type** — report inferences as the agent's own, attribute to the user only a message quoted verbatim; (c) **the `description:` must keep its USER-TRIGGERED ONLY clause** — a description reading "use after implementing a UI/UX feature" is a standing invitation to auto-spawn, and the description is what the caller reads when choosing an agent. Gets Chrome MCP tools, NOT `getDiagnostics`
...
- [ ] `task-builder` is `model: sonnet` and **omits `tools:` entirely** — deliberate (full set incl. `Agent`; the enum can't be appended to, so adding a list back silently revokes `Agent`). It is the ONLY agent without an allowlist, so its body's Scope Rules are the sole guard: verify the file-partition rule and the "pass your owned-file list to any sub-agent you spawn" row are both still present. Several run in parallel when a task splits by file, and two agents writing one file clobber each other with no error
...
- [ ] ⚠️ **Pruner size-policy migration — MIGRATE ON SIGHT.** Most existing projects still carry the superseded `### 0. Target length` step, which hardcodes its own ceiling (`~200 lines`, `350`, or a repo-specific number). That is **template drift, not a project-specific fill** — the number looks locally tuned, so the L194 carve-out reads as covering it and the file gets left alone. Replace the whole step with the template's `### 0. Size policy is NOT this agent's — delegate it`, and patch the Philosophy paragraph's stale "distinct jobs / distinct lanes" sentence in the same edit. Detect with `grep -n "Target length\|outer ceiling" .claude/agents/claude-md-pruner.md` — any hit means unmigrated. Rationale: `condense-claude-md` is the single source of truth for every size/split decision; the agent exists for `memory: project` + background spawning, not for owning a threshold
- [ ] ⚠️ **Pruner artifact-branch migration — MIGRATE ON SIGHT, same class as the row above.** Projects generated before the task-doc branch landed carry a CLAUDE.md-only pruner: no `### 0.5 Detect the artifact` step, one classify table, a NEVER-remove list with no task-doc invariants. Also **template drift, not a project-specific fill**. Port the whole template: widened `description:`, artifact-conditional Bootstrap (task-doc row invokes `Skill(read-summary)`), step 0's artifact-dependent delegate target, step 0.5, classify §2a/§2b, branched step 3 greps, and both NEVER-remove lists. Detect with `grep -c "0.5 Detect the artifact" .claude/agents/claude-md-pruner.md` — **0 means unmigrated**. ⚠️ Verify any detector you add in BOTH directions before trusting it (must hit an unmigrated fixture AND return 0 on a migrated file); a detector that fires on correctly-migrated files churns every project it touches
- [ ] **Template-drift check**: `diff` each `.claude/agents/<name>.md` against `templates/<name>.template.md` — every checklist item above verifies internal structure, none of them catch a template feature added after this project's agents were generated (e.g. a tool grant, a process step, a new section) that never got backported. A generated file can pass every other item here and still be stale. Backport genuine feature gaps; project-specific fills (rules tables) are NOT drift. An in-file model override is preserved **only if a justification comment accompanies it** — an unjustified deviation from the template's `model:` **is** drift, align it to the template. ⚠️ The mirror at DISPATCH time is invisible to any drift check: passing `model:` to a registered `subagent_type` silently overrides the tier its frontmatter sets, with no error. A registered agent's tier is a property of the agent, so the caller supplies `model:` only for `general-purpose` — or when the user explicitly asks for a tier, in which case say which tier is being overridden, since an agent inheriting the session model means an "override" may change nothing
- [ ] ⚠️ **Sub-heading drift — compare TABLE ROWS, not sections.** Every check above resolves at the heading or field level, so a generated file missing the tail of a table passes all of them: the heading exists, the table exists, only its last rows are gone. Truncation during generation makes this the most likely drift shape and the least visible one, and a table's final rows are where the load-bearing constraints sit. Diff the row labels of every table, per file — `diff <(grep -o '^| \*\*[^|]*\*\*\|^| [A-Za-z][^|]*|' <template>) <(... <agent>)` — and treat every template-only row as a gap. Frontmatter gets the same treatment: diff the whole block (`awk 'NR==1&&/^---$/{f=1;next} f&&/^---$/{exit} f'`), never a `grep -A1` on a multi-line YAML list, which truncates the list and invents a gap that isn't there. Rows present only in the generated file are project fill, not drift. Run all of it directly — a delegated "diff all N files" summary reports whole missing sections and silently drops everything finer
