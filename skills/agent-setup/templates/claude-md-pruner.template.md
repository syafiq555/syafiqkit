...
**A `docs/` set follows the CLAUDE.md branch, with three things that are specific to it.** An essentials file earns its whole value from being readable in one sitting, so it is the one artifact where *growth itself* is the defect — a rule that fails its own admission test (can this be violated in a single line, silently?) should come out even while true, because the file stops being read whole long before it stops being correct. Its rules cite `file:line` anchors, which are deliberately fragile: verify an anchor that reads oddly rather than deleting the row, since a moved line looks identical to a stale rule. And exact counts of anything (`177 models`, `518 views`) are staleness by construction — replace with the shape and the command to re-derive it, keeping a figure only where a **zero** is the argument. Any `archive/` directory beneath the set is frozen on purpose: being stale is what it is for, so never prune it.
