...
**Run `diff` against the template before this read, not instead of it — the full file, never a heading-only extraction.** A prose read-through, done carefully, still misses missing capability — a redelegation ban stated as a standalone sentence in the template can be absent from the generated file while a nearby, softer sentence reads close enough that a careful pass calls it present. One verification session read all eight agents this way, judged four clean, and only found real gaps in three of those four after running `diff` on each — the same missing-redelegation-rule shape recurred across five of the eight files and a careful read had passed every one of them. `diff` surfaces every line that differs; the reading skill from here on is triaging that output into "reworded, ignore" versus "capability absent, real finding" — but do the diff first, because the reworded lines are exactly what a read-through anchors on and stops looking past.

⚠️ **A heading-only diff (`diff <(grep '^#' a) <(grep '^#' b)`) is the same blind instrument as the frontmatter comparison this file already burned once — it resolves above the level a whole new section's content lives at.** A template can gain a genuinely new mid-body section between one session and the next (a rewrite, not a rename), and a heading-diff shows that as a changed/added heading line indistinguishable from an ordinary reorganisation — `## Review Lenses` renamed to `## What Counts as a Gap` reads exactly like `## Interrogate the Rules` appearing for the first time, because both are just a line that differs. Reading that output and classifying it as "structural renaming, content equivalent" without opening the body under each new heading is the prose-read-anchors-on-the-reworded-part failure one level up: this time the anchor is the diff's own reduced view, not the file. Confirmed live 2026-09-09 — a `product-reviewer` template gained a whole "Interrogate the Rules" business-analyst pass (7 new named checks) the same day the project's own agent was verified, a heading diff surfaced the section-name churn, and it was read as tailoring until the user asked "are u sure only that?" twice. Run the diff with no `grep`, no `-q`, nothing that reduces the file to a projection of itself; triage line by line, and treat every added or heavily-changed heading as a prompt to open both bodies before deciding it's a rename.
...
**`task-builder` has no `tools:` line at all.** That is deliberate and load-bearing: an absent `tools:` inherits every tool available to subagents, which is the only way to hold `Agent` alongside everything else, and writing any list back narrows the agent to exactly that list — so adding one silently revokes the spawn grant. This also means any sweep that enumerates tool grants by looking for a `tools:` block cannot see task-builder's at all and will report it unaffected.

**`disallowedTools: [Write, Edit]` on `product-reviewer` and `browser-verifier`.** This line *is* the read-only enforcement, and the reason is resolution order: `disallowedTools` is applied first, then `tools` is narrowed against what remains. Leaving `Write` and `Edit` off the `tools:` list does not block them, because omitting the line inherits the full set rather than granting nothing. This repo's own `product-reviewer` shipped without it, guarded only by a `# NOTE: read-only by design` comment that reaches whoever edits the file and reaches the agent as nothing. Verify a read-only agent by whether the enforcing line exists, never by whether its frontmatter matches a template — and check the spelling while you are there, since an agent's `disallowedTools` is camelCase where a skill's is hyphenated `disallowed-tools`, and the wrong one is inert rather than invalid.
...
**product-reviewer** — does it sort findings by whether a user is actually blocked, distinguishing must-fix-now from known-and-accepted from optional polish, so a reader can tell which class any finding lands in? And does it name who this software is actually for? The tier *names* are project vocabulary; the three-way distinction is the property. Two further checks, because a generated file can keep all three tiers and still review with one lens: it must say that journeys tracing clean is where the rules and presentation passes *begin*, and its Product Context table must carry this project's real audiences rather than the shipped placeholder comments — a reviewer with no named parties cannot ask whether a capability given to one belongs to the other, and that question is unanswerable rather than merely unasked.
...
## A Bootstrap row can cite a section that no longer exists

A row citing a *section* of a doc goes stale when that doc is restructured, and unlike a moved file it leaves the path resolving perfectly. `docs/ARCHITECTURE.md` still exists, the row still reads as live, and the section it promises — an open-questions table, a named subsection — is simply gone or renamed. The agent is sent to fetch something that will not be there, and nothing anywhere errors.

This is the common case rather than an edge one, because a doc gets reshaped far more often than it gets moved, and the reshaping session has no reason to think about the fleet. Measured 2026-09-01: two documents were rewritten to standard templates hours after the agents were wired to them, and four rows across `Plan`, `product-reviewer`, `code-reviewer` and `claude-md-pruner` pointed at sections that no longer existed.

So verify a section citation against the doc's actual headings rather than against the file existing. And where a row characterises what a doc *contains* (a tagging convention, a fixed template, a deliberately-kept record), re-read that too — a doc can keep every heading and change what a reader is allowed to conclude from it.
