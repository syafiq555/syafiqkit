...
⚠️ **Before believing any local failure, confirm the app is serving the code you just wrote.** A dev server that dies leaves the app serving its last built bundle rather than erroring, so specs run against a stale frontend and fail on selectors that exist in your source — which reads as a locator regression you caused. Check the artifact the app resolves through, not the server process: the process routinely outlives the file that makes it authoritative. This is the same trap `uiux`'s "Verify by Looking" covers for screenshots; a spec run is the other way a session reads a render.
...
Where the flag-gated config goes is itself a trap, because the natural home is the one the framework refuses. Slowing the run down and widening the viewport are per-*worker* settings, so `test.use({launchOptions: {slowMo}})` cannot sit inside a `describe` block — Playwright rejects it with `Cannot use({ launchOptions }) in a describe group, because it forces a new worker`, and points the error at the `test.use` line rather than at the block that made it illegal, which reads as a malformed options object and sends you rewriting the wrong thing. Put that `test.use` at file top level, gated by the recording flag; a per-test timeout raise, which the slowed run also needs, is legal inside the block via `describe.configure`. The general shape: anything the worker is launched with is file-scoped, anything about how a test executes can be block-scoped.

**Gate the framing, never the claim.** Reveal, pace and hold are camera-only and belong behind the flag; an `expect` does not, because gated it runs only when someone happens to record and the ordinary suite never exercises it. This is easy to do by accident, since the assertion that makes a closing frame meaningful — the badge is gone, the row is empty — is written *while* building the recording path and lands inside the same conditional. What makes it costly is that the surrounding spec usually proves the same outcome by a different route (an API read of the column the action writes), so the gap is invisible: the state is verified, but the rendering of it is not, and those are different code paths that fail independently. When a recording-only block contains an `expect`, hoist it out and leave only the framing behind.
...
⚠️ **Do that before the file reaches the user, and treat every label you attach as a claim needing the same check.** The frame test is worthless performed after sending, and the moment of sending is exactly where the run's pass count feels like it has already answered the question — so a green suite plus a written file reads as a verified deliverable twice over. It is neither: the count is evidence about assertions, and the file's existence is evidence the encoder ran. Naming is the second half and fails independently, because a hand-built context's video carries a content hash rather than a test name; pairing hashes to tests by finish time is a guess that yields confident, wrong labels, and the user cannot tell a mislabelled video from a broken feature. Measured 2026-09-03: five videos went out on the strength of "53 passed", one showing a modal and another the wrong tab, and the user had to ask what they were looking at. Where a still frame answers the question, send that instead of or beside the video — it is smaller, unambiguous, and you have already looked at it. **Tell: you are about to attach a recording whose contents you have not viewed this turn.**
