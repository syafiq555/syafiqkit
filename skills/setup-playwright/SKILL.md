...
⚠️ **Verify a recording by looking at a frame, not by the run being green.** Extract a frame near the end (`ffmpeg -sseof -0.6 -i video.webm -frames:v 1 out.png`) and actually view it before sending. A green suite masks several invisible failures: the dev server serving stale code; encoding stopping when a context closes before the subject lands on screen; a recording capturing the wrong half of the page because assertions resolve below the fold while locators stay off-camera. A still frame catches all of these where a pass count does not. Do that before the file reaches the user, not after — the moment of sending is where a green count makes an unwatched file look verified.
...
- **Name the file after the test.** A hand-built context's video lands under a content hash the runner reports nowhere. Route every context through one close helper so naming and the tail wait have a single home; a context closing by itself skips both, and the missing name is the visible symptom of a truncated recording.
...
A recording is a viewport, not the page: content below the fold is absent, and a modal taller than the viewport looks cropped. Scroll the subject into shot before the hold, behind the same recording flag, and make it a no-op otherwise.
