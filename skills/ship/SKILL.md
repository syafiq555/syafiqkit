...
**Run the whole chain in one turn — a delegated sub-skill returning control is the middle, not the end** (📖 `_shared/references/one-turn-chain.md`). Half-done here also leaves the deploy unverified. **Tell: your reply names a remaining step instead of performing it.**
...
⚠️ **`git push` pushes the WHOLE local branch, not just the commit you just made.** If `origin/main..HEAD` (before pushing) has more than the commit `/commit` just created, the push carries prior-session backlog along with today's work — a user who asked to ship "this" reasonably assumes only "this" goes out, and only discovers otherwise by asking "is this what we deployed?" after the fact. Before pushing, run `git log --oneline origin/main..HEAD` and if it's more than 1 commit, say so plainly in the Ship Summary's operator commentary (e.g. "this push also carries N earlier commits that were sitting unpushed: `<subjects>`") — don't wait to be asked.
...
**No deploy is a gate — not even the ship's own.** Kick off the CI/deploy check (`gh run watch`/a Monitor/a polling loop) and proceed straight to Step 5 — a deploy running is not a reason to sit idle, and this applies to every deploy that comes up during the ship (the primary one, a follow-up mid-ship fix, anything else). Come back to finish 4.1–4.2 (and the task-doc write in 4.5) once the background check resolves, whether before or after Step 5. Only block synchronously if the very next step genuinely needs that deploy's specific output.

**Before CI/prod verification, discover task docs owning each shipped commit's changed files — don't reuse whatever doc is already in context.** `git show --name-only <sha>` per commit, then `grep -rl` those filenames/feature keywords across `tasks/**/current.md` (never assume the doc you loaded earlier for unrelated work is the right one for a different commit in the same push). Read each match's `## Next Steps` for items that reference the shipped file/feature and are still open — a commit that *writes* a tool (a new artisan command, a migration helper) is often paired with an approved-but-unexecuted follow-up action logged in a doc that has nothing to do with the commit's own domain folder. Surface any hits in the Ship Summary's operator commentary, don't just note it and move on.
...
Generate a Google Chat-formatted release note. **Frame from the task doc, not the changelog.** The CHANGELOG lists changes per-item and reads each as a self-contained win — summarizing from it alone over-claims (e.g. "cleared all alerts" when the task doc's `Status:` is 🟡 partially-done with deferred work). The bigger picture — what the effort *was*, what's actually done vs deferred — lives in `tasks/**/current.md`.

**Scope the note to everything the PUSH carried, not just the commit `/commit` just made.** A push including prior-session backlog must cover those CHANGELOG versions too. Find the last-known-*deployed* version — the task doc's prior live version, or the CHANGELOG entry for the last SUCCESSFUL deploy's SHA, which is not always the previous commit — and include every entry between it and the current one.
...
3. **Lead with the accomplishment, not caveats.** Headline = what was done ("upgraded dependencies + hardened security"). Deferred/partial work is a short closing note, not a co-headline — don't bury the win under an alarming caveat section.
   ...
   **A changelog's shape is not a release note's shape — restructure, don't transcribe.** A changelog is a flat per-item log grouped by `Added`/`Fixed`, which is right for a developer record and wrong for an announcement: it splits one feature across two headings and gives a headline feature the same visual weight as a tooltip fix, so a reader can't tell what the release IS. Convert to: **(a)** a short plain-language lead saying what changed for a user and what they could not do before — the before-state is the context no bullet can carry, and it usually lives in the task doc's Problem/Decision, not the changelog; **(b)** the headline feature's own items grouped together regardless of which heading they came from; **(c)** everything unrelated demoted under a plainly-named catch-all. Bugs that are really "this feature now works properly" belong under the feature, not billed beside it. **Tell: your note is a flat list of peer bullets ordered by `Added` then `Fixed`, with no prose above it — or the reader asks what was actually shipped.**
4. **Pass the CHANGELOG entry's actual text into `gchat-format` — never paraphrase it from memory first.** Summarizing the read before formatting is where items silently drop or get replaced with generic one-liners the changelog never said; the skill's own condense step already does the WHAT-not-HOW trim, so feeding it raw text is both less work and more faithful. Before sending, count the entry's actual shipped items against the output bullets — a mismatch means an item got lost, not "cleanly merged." **Count against whatever structure the entry really uses (headings, or a bare `## <version>` + prose bullets); hardcoding heading names makes the check silently unrunnable, and a check that cannot fire reads exactly like one that passed.**

   **That count validates the note against the CHANGELOG — it cannot tell you the CHANGELOG is itself short. Reconcile the entry against the DEPLOYED DIFF before formatting.** The changelog is written by hand mid-work and routinely under-reports what actually shipped, so a note faithfully derived from it inherits every omission while every self-check passes. Diff the deployed range (`git diff --stat <last-deployed>..<HEAD>`) and scan the surfaces a user can perceive — notification/mail classes, user-facing copy and status labels, templates, form and button states — then confirm each has a line in the entry. Two cheap cross-checks: a commit subject claiming a count ("close 14 items") against the entry's actual item count, and each commit's body naming changes the entry never mentions. Add the missing items to the CHANGELOG first, then format from the corrected entry. **Tell: your item counts match perfectly, and you never opened the diff.**
5. Format using the `gchat-format` skill (convert to Google Chat syntax).
6. Copy to clipboard.
7. **If the project documents an issue tracker, post there in the same run — don't wait to be asked.** A ship's last mile is telling the team, and a release note that only exists in the reply is a note nobody reads. Check `CLAUDE.md`/`CLAUDE.local.md` for a tracker convention (list/board ids, card-per-date naming, required status/assignee) and follow it exactly: **search for an existing entry first and update it — never create a duplicate**. Multi-repo ships usually need one entry per repo, in that repo's own list. No documented tracker → skip silently. **Tell: the user asks you to update the tracker after you reported the ship done.**
8. **Render the formatted result as its own labelled, fenced block — after the Ship Summary table, never inline within it.** A release note that only shows up as a table row reads as buried, not as a standalone artifact the user can copy — see the Output template below.
