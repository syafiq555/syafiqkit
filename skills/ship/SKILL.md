...
⚠️ **Scope the note to everything the PUSH carried, not just the commit `/commit` just made.** If Step 3 found the push included prior-session backlog, the release note must cover those CHANGELOG versions too — "give me the full release note, not just the one I committed" is the user catching a note scoped too narrowly. Find the last-known-deployed version (the task doc's prior "LIVE on prod" version, or the previous prod-HEAD SHA's CHANGELOG entry) and include every `## [version]` entry between that and the current one, not just the newest.
