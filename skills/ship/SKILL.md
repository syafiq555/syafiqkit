...
⚠️ **Check WHO wrote the backlog, and treat another author's work as a decision rather than a summary line.** `git log --format='%an' <deploy-branch>..HEAD | sort | uniq -c` costs one command and changes what the ship is: releasing your own unpushed commits is a backlog, while releasing a colleague's unreviewed feature is a call that belongs to the user. The two are indistinguishable from a commit count, which is why the rule above — routing backlog to the Ship Summary — is not enough on its own: a summary line is something the user reads *after* the merge, and this has to be asked *before* it. Measured 2026-09-01: a prod merge carried 33 commits by three authors, three of them a colleague's 1,682-line rewrite of a picking screen plus a packing page and new audio assets, none of it on prod and none reviewed in that session. Name the author, the surface and the line count, and offer cherry-picking the session's own commits as the alternative — the user may well say ship it all, but that is their answer to give. **Tell: the range you are about to merge contains a commit whose author is not you.**

**If the merge conflicts, the default is keep BOTH sides.** Ship conflicts are usually two people appending to the same file in the same window, so picking a winner discards a colleague's real work and still reviews clean. 📖 `references/merge-conflict-resolution.md` before resolving anything — it covers the changelog case (both sides write under the same dated heading, and accepting both hunks duplicates it) and why counting entries is the only check that catches a dropped one.
...
⚠️ **Match the run to YOUR commit before reading its conclusion.** A poll that takes the newest run and waits for `completed` will happily return the *previous* commit's run — it is already finished, so the loop exits green on the first tick while your deploy is still queued. Nothing errors, and the summary then reports a ship that hasn't happened. Key the wait on the SHA or commit subject you just pushed, not on position in the list, and treat "green" as a claim to confirm on disk (the destination check below) rather than the answer. 📖 `references/ci-provider-polling.md` for the per-provider commands and the sibling-run traps.
...
**If the diff includes migrations, verify the schema changed.** CI green and a passing health probe don't prove the migrations ran — the pipeline that ships code and the pipeline that runs migrations are often different systems (container entrypoint, release phase, separate job, or manual). Ask the DB directly — the migration tool's own status output showing the new migration as run, or the table's shape showing the new column present. Code expecting a column that doesn't exist usually breaks on first use, not at boot, so missing migrations surface as production errors hours after the deploy reads green.

⚠️ **A data step whose CODE rides in this payload runs AFTER the payload lands, never before — and running it early fails silently as a success.** The instinct is to seed or backfill first so the new code never meets missing data, which is right whenever the step is already deployed and wrong exactly when the step is *part of what you are shipping*: the destination still holds the OLD file, so it executes that, prints its usual success line, and changes nothing. Nothing distinguishes that from a real run except reading the data back, and the no-op reads as a broken tool rather than as wrong ordering. Ask which side of the deploy the step's own source sits on before choosing the order. Where running late leaves a real exposure window, that is a cost to state and bound, not a reason to invert the order — inverting it produces the same window plus the belief that it is closed. 📖 `references/ship-deploy-verification.md` → **Seeded Reference Data**.
