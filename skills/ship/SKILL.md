...
⚠️ **Scope the note to everything the PUSH carried, not just the commit `/commit` just made.** A push including prior-session backlog must cover those CHANGELOG versions too. Find the last-known-*deployed* version — the task doc's prior live version, or the CHANGELOG entry for the last SUCCESSFUL deploy's SHA, which is not always the previous commit — and include every entry between it and the current one.
