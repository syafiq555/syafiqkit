...
3. **For each repo with staged changes**:
   ...
   - **Task doc staleness gate** — ⚠️ MANDATORY, staged or not. `grep -n -i "uncommitted\|not yet pushed\|pending"` the doc. **Any hit → run `task-summary` as the very next tool call.** The grep hitting is the ENTIRE trigger; no judgment about whether the hit is "real" staleness is permitted. The rationalizations this blocks — "it's accurate right now", "expected transient state", "it'll become true once I commit" — are all *true* in the moment (of course a pre-commit line says "uncommitted"), which is exactly what makes them a trap rather than an exception. **Lexical carve-out (shape, not meaning):** a hit fused into an identifier or UI-label — camelCase/kebab/Pascal or a quoted element name (`pendingStep`, `pending-step chip`, `PendingTasks`) — is the domain's vocabulary, not a staleness claim; note it in your response and move on, no `task-summary` run — the identifier stays in the doc, so do NOT re-grep it to zero. This tests token *shape*, never sentence meaning, so it doesn't reopen the no-judgment rule above. Word boundaries don't decide it (`\bpending\b` still matches `pending-step chip`); anything not unambiguously code-shaped is prose — the absolutism applies in full. On a real-staleness hit (not carve-out): re-stage, sweep the WHOLE doc (LLM-CONTEXT line, Quick Start, `## Files`, Task Status table, `## Last Session` all repeat the same claim), and re-grep to zero. Skip only if no `tasks/**/current.md` overlaps this diff's files.
     **Under `/ship` this gate has an override — do NOT resolve a hit by writing the pre-deploy state.** See `skills/ship/SKILL.md` Step 2.
     **This gate's `task-summary` run satisfies `/done` Step 4 — but only if it actually ran one.** A gate resolved entirely via the lexical carve-out invoked nothing, so `/done` Step 4 still needs its normal bare scan; a later `/done` goes SCOPED to new findings only when a real run occurred. See `skills/done/SKILL.md` Step 4.
   - **Cross-doc status mirror sweep** — ⚠️ MANDATORY, separate from the file-overlap check: `grep -rl` the feature name/keyword across `tasks/**/*.md` for OTHER docs that characterize this feature's status inline (e.g. "deferred pending backend work") even with zero file overlap. Read the surrounding sentence — a bare `Related: tasks/x/current.md` link is neutral, but a status-characterizing sentence isn't, even without the word "uncommitted." Flip stale "deferred"/"scoped"/"not built" language to match what just shipped. ⚠️ **Sweep every repo this commit spans, each from its own root** — a sibling checkout's `tasks/` tree is invisible to a grep rooted in this one, and a feature built across two repos routinely leaves its stalest claim in the other repo's docs (an item saying "X has no Y" that this very commit added).
