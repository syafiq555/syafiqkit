...
2. **Changelog gate**: if staged changes include user-visible fixes/features/improvements, add an entry to the **workspace-root `CHANGELOG.md`** — the top-level one above all sub-repos — not the sub-repo's own, even when committing inside a nested repo. This applies whenever sibling repos share one root workspace; a standalone single-repo project updates its own `CHANGELOG.md` instead.
   ...
   - An entry that already exists is a claim to read against the diff, not a gate already passed. Follow-up work on a branch commonly *changes* what an earlier commit's entry described (a unit-per-invoice rule reversed to one invoice per cycle) rather than adding beside it, and the entry's presence is exactly what makes the check feel done. Both commits ship as one unshipped feature, so rewrite the sentence the diff contradicts; a second bullet beside a now-false one leaves the release note stating both.
...
3. **For each repo with staged changes**:
   ...
   - **Check task docs**: `git diff --staged --name-only | grep '^tasks/'` — read any staged `current.md`. Its `Status:` line and `## Last Session` reveal what was actually built. Also check `tasks/**/current.md` for docs whose *files* appear in the staged set even if the doc itself isn't staged. A staged `decisions/*.md` is the reasoning behind the diff and usually the best source for the body — read it before the code.
     - A doc's status vocabulary belongs to that doc's own feature. "Phase 3 complete, Phase 4 pending" read off a neighbouring domain's doc names *its* phases, and lands in the message as a roadmap for the feature being committed. The body states what the diff does; a status claim goes in only where the diff itself is that status.
   - **Read `git log --oneline -5` before writing the subject.** On a feature branch the first commit already names the feature, and a subject derived from the diff alone restates it — the log then shows the feature landing twice, and nothing in the diff warns you. Where a recent subject already claims the feature, this subject says what the batch adds on top of it.
     - A staged doc from a domain your code didn't touch is easy to write off as riding along, and the write-off is safe only if the doc records BUILD state. Some record LIVE state instead — fixtures bought on production, a flag set on one environment, a row someone will look for. That is a standing fact about the running system rather than a note about this session, so it survives into whatever ships next and is worth carrying forward even when the commit itself is unrelated to it.
