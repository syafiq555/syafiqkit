...
1. **Read the targets' uncommitted diff** — work already present from another session will freeze into your baseline as if it were the committed state, and every downstream check then measures the agent against wrong-starting material. Name any contested files in the prompt as context. 📖 `../_shared/references/diff-ownership.md` distinguishes what to attribute to whom. ⚠️ **Your own recent write is the same hazard from a source you won't think to check** — and it arrives in two shapes, of which the second is the one that keeps recurring. A re-dispatch after a *failure* is obviously mid-correction: an agent reading the target between a bad write and the revert works from the truncated file, then reports honestly against *its* baseline, so "no rows dropped" is true and worthless (measured 2026-09-01). But a patch applied after a pass you judged *successful* is invisible in exactly the same way while feeling nothing like a correction — the pass finished, you fixed one thing it got wrong, and the tree reads as settled. Measured 2026-09-11: a restored doc header was silently re-gutted by the next sequential agent, which had already opened the file, and the same block needed restoring twice in one run.
...
⚠️ **For research dispatches, checks 1–4 below all no-op — there is no artifact to diff, so the prose is the only thing to grade.** Open two or three cited sources yourself before relaying, and for any figure find the number *on* the page rather than confirming the page resolves. Treat a negative finding as the one claim whose evidence is an absence: retest a couple of the reported failures before repeating a conclusion drawn from them. **Tell: the report concludes something does not exist, and your evidence for that is the report's own failed fetches.** 📖 `references/verifying-research.md`
...
⚠️ **Internal contradiction is the easy case; the common one is a report that is perfectly coherent and simply wrong about the file.** Nothing in it argues with itself, so there is no seam to notice — the number is just false. Measured 2026-09-11 across one four-file refresh: a report said "75 lines" of a 48-line file, a second asserted "all 6 companion pointers valid (consolidated, not deleted)" where 11 pointer targets had become 7, and a third reported a protected block preserved while having cut it. All three read as careful and itemised. So the trigger for re-counting is **every** report that states a number you would otherwise repeat, not only one whose figures disagree with each other — and the count to take is the one the report's own claim rests on (pointer targets, not pointer mentions; distinct anchors, not heading count).
...
⚠️ **Overshoot is the same miss and arrives labelled as a win, so the framing that should trigger the check is the one that suppresses it.** An agent briefed to cut to a *range* reads the number as a floor to beat and reports the overshoot as overperformance, where undershoot at least announces itself as unfinished. The remedy is the opposite of a re-dispatch, since the work is done and possibly over-done: rank the returned files by how far each landed from its brief and spend the meaning-read budget on the outlier first, in whichever direction it missed. Measured 2026-09-10, four files condensed against a 40–55% brief: three landed inside it, the fourth cut to 28% and was the only one to drop an actionable rule — the ratio told you *where* to read hardest, never whether damage had occurred. **Tell: an agent's report states its result as exceeding the target rather than as landing in it.**
...
Doing this in the other order is what fails. A symptom list — whole sections gone, contradicting numbers, an untrustworthy report — reads as authorisation the moment one item matches, and the enumeration test placed after it never gets applied to a case that already qualified. Those symptoms establish that something is wrong; they never establish that the rest is worthless.

Three ways a patch goes wrong after that call is correctly made, each invisible to the check that authorised it. **Tell: your evidence for "systemic" is a set of greps you chose, and you have not opened the rewritten file's sections.** **Tell: you are restoring a passage because it reads as more detailed, not because you can name what a reader loses without it.** And a fact restored into the section the agent happened to leave it in inherits that placement as correct — true, well-worded, wrong section, which the fact-survival check cannot see.
