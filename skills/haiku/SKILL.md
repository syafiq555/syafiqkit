...
**A prompt that says what to REMOVE gets applied to whatever resembles it, so state the boundary as a positive.** Justifying the removal by what a thing *assumes* is the shape that spreads: "`php artisan` assumes Laravel" is true of the tool and false of the project, so agents given that reason strip the framework's own command names and every report comes back successful, because each deletion genuinely matched the reason it was handed. The same instruction was correct on one corpus and destroyed 400 identifiers across another. Name which side of the line a case falls on — *hardcoded platform syntax goes, names of things that exist here stay* — rather than describing what the wrong side has in common, and expect to verify by count rather than by report.
...
**Phase 2: Did meaning survive?** Read the file whole, as its reader meets it — not the diff, not the passages the report names. A report describing a rewrite steers you to exactly the places it feels confident. Counting anything at this stage measures shape, and defects usually live in meaning, so the read is the real check on what changed in place.

**The exception is a fan-out too wide to read**, and it inverts that advice rather than softening it. Across a dozen files the whole-read stops happening — each one reads coherently, so nothing draws attention, and the pass that removed too much is indistinguishable from the pass that removed exactly right. There the cheap count is what you have: name the class of thing the rewrite must NOT touch and count instances before and after, per file. A rule aimed at prose will take identifiers with it, so command names, hostnames, class and method names, config keys and flags are usually that class. One such sweep found 400 removed across 55 files after nineteen reports had described the same work as a clean conversion — because a summary phrases the correct pass and the destructive one identically, and the count is the only thing that separates them. Then read whole the two or three files the count says lost most.
