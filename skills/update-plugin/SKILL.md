...
⚠️ **Stating this rule is not enough — a mental "re-scan" still defaults to whatever's freshest in context.** This exact skill has produced a partial scan twice in one session: first pass found 1 of 3 signals, second pass (after "you missed one") found 2 of 3, both despite reading the warning above each time. The fix is a concrete artifact, not a stronger intention: **before writing anything, list every distinct user message in the conversation as a numbered line (one line per message, in order, starting from message 1)** — not a summary, an actual enumerated list. Only after the full list exists, mark which lines contain a correction/signal. A signal found by re-reading "the recent part again" instead of walking this list is the failure mode recurring. If you cannot produce the numbered list (context was compacted, transcript unavailable), say so explicitly rather than silently scanning what's left.
