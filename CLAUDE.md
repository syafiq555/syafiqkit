...
- **The rewrite skills branch on who owns the target, and both branches are deliberate**: a CLAUDE.md inside this repo gets house style enforced, one in a consumer's repo gets its own convention adopted. Neither is the "real" rule to collapse the other into — imposing house style on a consumer reshapes work that was never ours, and deferring inside this repo lets a bad shape survive every pass run to correct it. The asymmetry lives in `update-claude-docs` (Rewrite gate + the Capture write step), `condense-claude-md`, and `_shared/references/adopt-vs-impose.md`; task docs are explicitly carved out, since `tasks/**` never ships and so has no consumer side. The load-bearing sentence is that **consistency proves a pass was uniform, never that it was right** — which is why a uniformly-wrong shape in a file we own is drift rather than a convention to respect.
