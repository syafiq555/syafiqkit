...
Last updated: 2026-07-22 — see Quick Start / Last Session (v1.122.1: version-drift auto-check added; absence-gate + split-index fixes)
-->

# Plugin Maintenance

**Status**: Reference (ongoing) — index for a whole-doc MADR decision log split by theme into `decisions/*.md`. Current version: v1.122.1.

## Quick Start (read this first in next session)

**Where we are**: Plugin is a mature skill/command system (24 skills, 2 commands, 8 agent templates), v1.122.1. Latest changes since v1.120.0: (1.122.1) `condense-task-doc` Step 9 now points a still-over-budget whole-doc MADR back to Step 2's split trigger instead of prescribing a fruitless second compression pass — also fixed `plugin.json`/`marketplace.json` version drift (3rd occurrence, see Next Steps for the gate this triggers); (1.122.0) `merge-task-docs`/`condense-task-doc`'s split-index pattern was silently dropping `Task Status`/`Bugs Fixed`/`Critical Gotchas`/`Next Steps` — the index keeps 3 things (Quick Start + doc-wide operational tables + routing table), not 2 — fixed both skills' split branches and added an absence gate (every EXCESS-detecting check misses a dropped section type; `merge-task-docs` Step 7 now diffs section headers source-vs-index); (1.121.1) `condense-claude-md`'s companion-file split can ship machine-specific `~/<user>/...` paths into a tracked `shared/` file — now greps the moved block first; (1.121.0) `read-summary` Step 3 no longer lets a staleness finding expire on an unanswered offer — hand off in the same turn or carry it as an owed action; added Step 1 sweep-don't-spot-report (Quick Start/Next Steps are revisited least, so hold the costliest staleness). Before that: `sweep-doc-overlaps` — fleet-wide parallel `Explore` scan across all `tasks/` domains for cross-domain merge candidates `merge-task-docs` alone can't find; discovery only, hands off to `merge-task-docs` for execution.
...
- A split index (`current.md` + `decisions/<theme>.md`) keeps THREE things, not two — Quick Start, doc-wide operational tables, and a routing table. Only per-theme detail moves down; dropping `Task Status`/`Bugs Fixed`/`Critical Gotchas`/`Next Steps` is invisible because every existing gate detects excess, never absence — see D41 (decisions/doc-condensation/structural-splits.md)
...
- **v1.121.0-1.122.1 shipped as a docs-only `/done` run** — no application code touched, only SKILL.md rule edits + CHANGELOG + version bumps across 4 releases. 1.121.0: `read-summary` Step 3's staleness handoff could be satisfied in letter while dying on an unanswered offer — now hands off in the same turn or carries the correction forward; added Step 1 sweep-don't-spot-report (Quick Start/Next Steps hold the costliest staleness, revisited least). 1.121.1: `condense-claude-md`'s companion-file split could ship a hardcoded `~/<user>/...` path into a tracked `shared/` file verbatim from the source section — now greps the moved block for absolute paths first. 1.122.0: `merge-task-docs`/`condense-task-doc`'s split-index pattern paraphrased `templates.md`'s 3-part index rule (Quick Start + operational tables + routing table) down to 2, silently dropping `Task Status`/`Bugs Fixed`/`Critical Gotchas`/`Next Steps` on every split — confirmed on a real merge where 3 template-compliant source docs lost all 4 sections and no gate caught it (every check there is subtractive, none detects absence). Fixed both split branches + added `merge-task-docs` Step 7 absence gate (diff source vs index section headers). 1.122.1: `condense-task-doc` Step 9 prescribed "run a second aggressive pass" even when the excess was MADR blocks with nothing left to compress — now routes back to Step 2's split trigger instead; also caught `plugin.json`(1.122.0)/`marketplace.json`(1.121.1) version drift mid-session, its 3rd occurrence (see Next Steps).
- **This doc reconciled from v1.120.0 → v1.122.1** — 3 releases stale (Quick Start, version header, Gotchas). Caught during this same `/done` run's docs-only referential-integrity check.
...
- [ ] `plugin.json`/`marketplace.json` version drift hit its 3rd occurrence (D26 2026-07-15, 2026-07-17, 2026-07-22) — the "consider a gate" threshold this item was watching for. Still no automated gate; a pre-commit check or single-source-of-truth version file is now due, not just worth considering
