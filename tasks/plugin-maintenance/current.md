...
| Whole-doc MADR is a distinct choice from per-decision MADR, priced differently | The per-block rule ("+18-20 lines, use sparingly") measures ADDING a block beside existing tables. A whole-doc rewrite REPLACES the Decisions + Gotchas tables — folding each gotcha into its owning ADR's Consequences removes the decision-vs-gotcha cross-section duplication, so a decision-heavy doc converts SHORTER (measured 307→284 on a 13-decision/27-gotcha doc), not +200. `task-summary` now branches whole-doc structure on the doc's primary value: decision-traceability → full MADR log; operational cold-start → standard. Only on explicit user request; never default. |
...
| Generated agents INVOKE `/read-summary` for task-doc discovery, they don't reimplement it | Same "orchestrator delegates, never inlines a sibling's procedure" principle, applied to `agent-setup`'s output. Agent templates were hand-copying read-summary's Glob-`tasks/**/*.md`+Grep-vocabulary logic into each Bootstrap — which drifts (sibling-repo path, synonym rule, `Related:`-follow all restated, all going stale independently). Fix: every task-doc-consuming agent (all but `claude-md-pruner`, which is CLAUDE.md-only) carries `Skill` in its `tools:` and names `/read-summary` as canonical, with a short inline fallback for when the skill can't be invoked. Also caught the drift's tell: `Explore`'s `description:` claimed it "reads task docs" while its Bootstrap never mentioned them (frontmatter drives routing, body drives behavior — disagreement = under-delivery). Patched SKILL.md (Key-rules + Step-5 verify checkbox) AND all 5 templates so a regen doesn't drop it. |
| Correct wiring to invoke a sibling skill isn't the same as the model reliably calling it | Follow-on to the row above: 1.55.0 gave `Explore`/`Plan` the `Skill` tool and named `/read-summary` in Bootstrap, but a real session still skipped the call — a free-text prompt that already named exact files/methods for an OAuth flow read as a fully-scoped code trace, so the agent went straight to CLAUDE.md reads. Bootstrap instructions are advisory prose an agent can judgment-call around; wiring alone doesn't force the call the way a hook or schema constraint would. Fix: added an explicit counter-instruction to both templates — prompt specificity implies a task doc is MORE likely to exist (the caller's detail came from somewhere), not a reason to skip discovery. Only `Explore`/`Plan` needed this; `code-reviewer`/`code-simplifier`/`product-reviewer` trigger off `git diff`, not free-text prompts, so they aren't exposed to the same misjudgment. |
