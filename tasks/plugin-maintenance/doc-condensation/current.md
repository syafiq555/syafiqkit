...
Last updated: 2026-08-02 — `/done`'s own review pass caught a real D66 collision (two unrelated decisions independently minted the same id; renumbered one to D67 — see Last Session) and a false-0 in `read-summary`'s router-detection grep, verified only against an external project's ID convention at 1.137.0. Prior same day: a 3rd instance of D64's shape found: `unhobble-instructions`' decoration-vs-content check existed but wasn't wired into its own executable Process steps, so a self-run mis-stripped a genuine Tell (full record in ../agent-architecture/current.md, no new decision here). Prior still: D67 — `condense-task-doc`'s aggregate line/byte target passed while individual sentences stayed 500+ chars — fixed with a direct per-line length check (step 11)
-->
...
- D68: the unhobbling pass now reads the file as a document before any rule, because its own per-rule framing let a session degrade the pass into a marker grep — the user caught it mid-run. The three shape defects it then found (nine rules × three homes in `task-summary`, two files violating their own stated rules, `done`'s defenses-as-prose vs derivable-as-tables) were all invisible to a keyword scan.
- 1.137.17's relocation of `update-claude-docs`'s arrival-rate callout orphaned a "the MANDATORY callout above" reference; fixed by moving the rule back above its citing row, since Capture is the mode that adds lines.
- `/done`'s own review pass caught the ADR-id gap in Next Steps (below) producing a real collision: `condense-task-doc`'s per-line-length fix and the unrelated unhobbling-authoring-default decision had both minted `### D66` independently — the former scoped its "next free id" grep to its own file's history instead of global `tasks/`, exactly the anti-pattern this doc's own CLAUDE.md rule already names. Renumbered the per-line-length fix to D67 and fixed all six citations; `grep -rhoE "^### D[0-9]+" tasks/ | sort | uniq -d` now returns empty.
- Same review pass found `read-summary/references/doc-authority.md`'s router-detection check (`grep -c "^### D-"`, added at 1.137.0) silently returns 0 — a false "this is a router" — against this plugin's own hyphen-less `### D<n>` convention; it was verified at 1.137.0 only against an external project's `D-N` style. Widened the pattern to `grep -c "^### D-\?[0-9]"` and confirmed it now returns 10 (not 0) on a real ADR file in this repo.
