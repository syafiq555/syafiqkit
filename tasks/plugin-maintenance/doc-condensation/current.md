...
- D-haiku-condense-delegation: reversed the blanket "no spawned agent" condensation-drafting rule now that a new `haiku` skill exists to own dispatch + mandatory post-return verification. Measured on a real 375-line CLAUDE.md condense: 35% byte cut, rules intact.
- `/done`'s own review pass (product-reviewer) caught the first draft of this change describing delegation as *possible* without ever stating the dispatch call — `two-tier-condense.md` now names `Skill(skill: "syafiqkit:haiku")` explicitly in its Draft section.
- Two agents (code-simplifier, product-reviewer) independently flagged the same duplication from different angles: `haiku/SKILL.md` restated `explore-delegation.md`'s prompt-writing guidance and `two-tier-condense.md`'s verification steps instead of pointing to them. Fixed by collapsing both blocks to pointers.
- New failure mode named across `two-tier-condense.md`, `unhobble-instructions/SKILL.md`, and `haiku/SKILL.md`: a delegated rewrite can keep a backtick-quoted identifier intact while reversing the claim around it (kebab-case keys relabelled "camelCase"), which survives an identifier-survival grep untouched.
