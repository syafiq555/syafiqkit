...
- **`references/*.md` size policy decided** (was deferred twice): out of scope for the ~90 B/L gate — that ratio measures a hot path, while a reference is a cold-path lookup whose correct shape is a dense table. A reference owes single-topic + a symptom-naming pointer + **~6KB for prose**; **catalogs are exempt** and grow with what they catalog. The test is how the file is READ.
...
- Extraction stopped at 3 of 6 candidates on purpose: `read-summary`'s ratio *rose* post-extraction (199.8→206.8), D50's documented floor signature. The queued ~130 B/L projection was stale — it assumed 4.9KB would move and only 621 bytes did, because the marker pass had already shortened those lines.
