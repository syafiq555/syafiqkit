...
Last updated: 2026-09-12 — backlog pass. Version-drift gate CLOSED after 10 recurrences (`.githooks/pre-commit`, opt-in per checkout). Four more rows closed: the resident-vs-lazy-load extraction declined on measurement (D-a-shared-vocabulary-is-not-a-shared-rule — four files shared a vocabulary, not a rule), size-measurement drift already fixed (all three gates measure lines against 300), D65's reciprocal note added to `unhobble-instructions`, and `structural-splits.md` given a declared ~10KB budget. Found while working: the plugin's own ceiling measurement was wrong, and it was a RECURRENCE — 1.261.0 disproved `bytes/4` and patched the figures while leaving `harness-constraints.md` prescribing it for ten more versions. `harness-constraints.md` prescribed `bytes ÷ 4` as the token count; it over-reports this corpus by ~12%, and 17% on a file as dense as this one, so `unhobble-instructions` was reported at ~6,000 tokens when the real `cl100k_base` count was 5,119, and three skills read as over the ceiling when **all 38 are under**. Divisor replaced with a tokenizer command at source; the inflated figures corrected here and in `external-guidance`, where `agent-setup` sat recorded as "marginally over". Earlier same day — condensed Next Steps (55→12 lines), Prior Sessions (31→5 lines). Doc set was 1,207 lines / 163 KB (3.5× budget); split is correct structure, decision files are dense-but-live MADR blocks. Arrival-rate gate watching 1.33:1 ratio.
-->
...
- **The bytes/4 error was a RECURRENCE, and the first fix is why it survived.** 1.261.0 diagnosed this exact bug, quantified it ("overstates English prose by 15–20%") and named four of six files as never over the ceiling — then patched only those figures. `harness-constraints.md` kept prescribing `bytes ÷ 4` from its creation (9f38506) for another ten versions, until a session re-derived the whole thing from scratch. **Correcting a number does not correct the rule that produced it, and a corrected number reads as a closed case.** Found by the product reviewer, not by this session's own sweep.
- **One correction pass missed a second figure derived from the same bad divisor.** 1.270.0 recorded the file at "roughly 5,400 tokens" post-fix; it was 4,566. The retraction fixed the peak figure and left the resulting one, so a reader of that entry alone still believed the file sat near the limit. After disproving an estimator, grep for *every* number computed under it, including in write-ups already shipped.
- **A ratio measured on one file generalises badly.** The corpus averages 4.46 bytes/token (~12% over-report); 17% was `unhobble-instructions` alone, the densest file in the sample, and it got written into five places as the corpus figure. Corrected everywhere.
- **The version-drift hook is opt-in and nothing said so where it mattered.** `CLAUDE.md`'s Version Bumping section — the one place a session reads before bumping — never mentioned `.githooks` or `core.hooksPath`, so a fresh clone started unprotected, and `ship`'s print-step covers only the ship path while a bare `/commit` has neither defence. Cross-referenced now.
- **The reported ceiling breach was a measurement error.** `unhobble-instructions/SKILL.md` was called ~6,000 tokens from `bytes/4`; the real `cl100k_base` count was 5,119 (119 over, not 1,000) and is now 4,566. Swept all 38 skills: **none exceed 5,000 real tokens**, while `bytes/4` flags three. The estimator errs one way only, so it always presents as a breach to fix rather than as a bad ruler — and the reorder pass it motivated was still worth doing, which is what made the bad number easy to keep.
