...
Last updated: 2026-08-14 (v1.149.0) — parity drift runs in both directions at once: `claude-md-pruner`'s template held newer prose while its generated agent held a better guard placement, so "patch the template in the same change" names the wrong repair. Decide per hunk (D-parity-drift-is-bidirectional). Two sessions collided on that same fix in a shared checkout; the peer's structure was kept and their renumbering defect fixed. Session-by-session history lives in `## Last Session` and the ADRs it cites — not here.
-->
...
**Where we are**: How generated project agents (`.claude/agents/*.md`) inherit CLAUDE.md conventions, delegate to sibling skills, reliably invoke them, and how the plugin delegates work to cheaper/parallel agents — plus how a session learns a *peer session* is writing the same files. 36 decisions (35 live, D35 superseded; D-quick-done superseded in its step composition only) across 3 themed sub-files (counted, not incremented — `grep -h '^### D' decisions/*.md | wc -l`).
...
- `/done` over 5 unpushed commits (an `unhobble-instructions` sweep across done/ship/commit/task-summary/update-claude-docs/condense-*) plus a working tree carrying a second session's live edits. Shipped at 1.149.0.
- **Parity drift is not always one file being behind — both sides can be ahead on different axes at once.** `claude-md-pruner`'s template held the unhobbled prose while the generated agent held a better guard placement (never-remove moved into classification rather than trailing the file). D38 covers "both files wrong and matching"; this is the case where each is right about something different, and the standing "patch its template in the same change" instruction produces the wrong repair — a wholesale copy either direction discards the other side's improvement, and copying *toward* the template also ships repo-specific text upstream into a file meant to be generic. New ADR in `decisions/verification-rigor.md`.
