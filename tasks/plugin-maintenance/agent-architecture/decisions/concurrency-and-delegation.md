...
- Out of scope by user decision: a version-file disagreement (`plugin.json` 1.140.5 vs `marketplace.json` 1.140.4) left in the working tree by a prior fan-out race, observed while planning. Reported, not fixed — no parity check added, so the same working-tree shape recurred at 1.173.0 and was again reconciled by taking the next number (D-guard-the-act-not-the-dispatcher). Worth noting what the recurrence taught: a disagreement seen in the working tree says nothing about what shipped, and reading one as shipped drift is a claim `git show <sha>:<path>` across the history disproves in one command.
